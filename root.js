...
var API = function(configuration) {
  ...
  */
  ...
  if (this.configuration.root){
    config.push(this.configuration.root);
  }

  // console.log("config:" + util.inspect(config, {depth:3}) + "end");

  Async.eachSeries(config, function(root, callback){
    self.createRootDevices(root, callback);
  },
  function(error){
...
API.prototype.createRootDevices = function(config, callback) {
  ...
  Async.eachSeries(config, function(root, callback){
    ...
    new rootdevice(self, root, function(error, instance) {
        if (error) {
          logger.error(error);
        }
