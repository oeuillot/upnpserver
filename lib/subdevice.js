...
/**
 * Require service file, instanciate pass configuration
 *
 * @param {object}
 *        configuration
 * @param {string}
...
SubDevice.prototype.addServiceInstance = function(service, callback){
  ...
  self.makeServiceInstance(configuration, service.classPrefix, function(instance){
    self.addService(instance, callback);
  }.bind(self));
}
...
SubDevice.prototype.addService = function(service, callback) {
  var self = this;
  service.initialize( function(error) {
    ...
    // service instances shall not advertise on ssdp
    if (service.route.indexOf("_id_") < 0){
 ...
 */
SubDevice.prototype.toJXML = function(segment, request, callback) {
...
/**
 * override this in your device to handle vendor ssdp headers
 * eg: heads[vendor extension] = vendor value;
 *
 * @param {object}
             headers
 * @param {bool}
 *            alive
 */
SubDevice.prototype.ssdpHeadersCallback = function(heads, alive) {}
