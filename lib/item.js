...
var Item = function(parent, name, upnpClass, container, title) {
	...
	if (log) {
		console.log("NewItem " + this.itemId + " parent="
				...
				+ " upnpClass=" + upnpClass + " container=" + container
				+ " title=" + title);
	}
...
Item.prototype.listChildren = function(callback) {
	if (!this.container) {
		if (log) {
			console.log("Item.listChildren => No container");
		}
	...
	if (this._children !== undefined) {
		if (log) {
			console.log("Item.listChildren => cache ", this._children.length);
		}
	...
	if (log) {
		console.log("Item.listChildren => not in cache !");
	}
...
Item.prototype.getService = function() {
	...
	console.error("Can not find root node");
...
Item.prototype.getChildByName = function(name, callback) {
	this.listChildren(function(error, children) {
		if (error) {
			if (log) {
				console.log("Item.getChildByName(" + name + ") => error ",
						error);
			}
			return callback(error);
		}
...
Item.prototype.treeString = function(indent) {
	indent = indent || "";

	var s = indent + "# " + this.itemId + " " + this.name + " "
			+ this.upnpClass + "\n";
	if (!this.container) {
		return s;
	}
