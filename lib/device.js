...
Device.prototype.makeServiceInstance = function(configuration, name, callback){
  ...
  try{
    instance.ErrorSoap = require('./util/' + name + 'ErrorSoap');
  } catch(e){
  ...
  this.findPort(this.configuration.httpPort || 8080 ,function(port){
    ...
    server
      ...
      .on('listening', function(err){
        ...
        // add multihomed device adresses
        self.ips.forEach(function(host){

          var location = 'http://' + host + ':' + self.port + '/description.xml';
          self.ssdp[host].addDevice(self.uuid, self.type, location);
...
Device.prototype.addService = function(service, callback, advertise) {
  ...
  service.initialize(this, function(error) {
    ...
    if (advertise !== false){
      ...
      // add multihomed device address for services
      self.ips.forEach(function(host){
          self.ssdp[host].addService( self.uuid, service.type);
      });
  ...
  var xml = {
    ...
    _content : {
      ...
      device : {
        ...
        UDN : this.uuid,
        presentationURL : "http://" + request.myHostname + ":" + this.port + "/index.html",
