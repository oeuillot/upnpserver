...
AVTransportService.prototype.processPlaylist	= function(raw, index, callback){

    var self = this;

    var xml = new xmldoc.XmlDocument(raw)
    ,   res = []
    ;


    self.playlist.splice(index, res);
...
AVTransportService.prototype.getCurrentTrack	= function(index){
	...
	,	trackIndex	= self.stateVars.CurrentTrack.get()
	;
	if (index != undefined) trackIndex = index;
	if (playMode == 'SHUFFLE' || playMode == 'SHUFFLE_NOREPEAT'){
		trackIndex 	 = self.shuffle_indexes[trackIndex];
		}
	return trackIndex;
	}
...
AVTransportService.prototype.playNext = function(witch){
  ...
  // self.setCurrent(media, currentTrack);
  self.offset = 0;
  //	args  = [self.instanceId, media.res[0].uri, '00:00:00.00'];
  //	if (len) args.push(len);

  self.stateVars["AVTransportURI"].set(uri);
  self.stateVars["AVTransportURIMetaData"].set(meta);
