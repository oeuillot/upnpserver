...
var  AVTransportService = function(device, classPrefix, configuration) {
  ...
  this.addType("LastChange", "string", 600, "",
      [], null, true, 0.2, null, function(){
        // lastChange JXML are stored on root instance
        ...
        var instance = self.device.services[route];
        // LastChange <event> node should be xml escaped string
        self.stateVars["LastChange"].value =
          jstoxml.toXML(instance.lastJXML, {
            header : false,
            indent : " ",
            filter : xmlFilters
          });
        instance.lastJXML = null;
      }, function(){
        self.stateVars["LastChange"].value = null;
      });
  this.addType("TransportState", "string", 600,"STOPPED",
      [ "STOPPED", "PLAYING", "PAUSED_PLAYBACK", "PAUSED_RECORDING",
        "RECORDING", "NO_MEDIA_PRESENT" ], null, 2);
  ...
  this.addType("RecordMediumWriteStatus", "string",600, "NOT_IMPLEMENTED",
      ["WRITABLE","NOT_WRITABLE", "PROTECTED", "UNKNOWN", "NOT_IMPLEMENTED"], null, 2);
  this.addType("CurrentRecordQualityMode",713, "string", "NOT_IMPLEMENTED",
      ["0:EP","1:LP","2:SP","0:BASIC","1:MEDIUM","2:HIGH","NOT_IMPLEMENTED"], null, 2);
  ...
  // possible actions for current media
  ...
  this.addType("A_ARG_TYPE_SeekMode", "string", 710, "TRACK_NR",
      ["TRACK_NR","ABS_TIME","REL_TIME","ABS_COUNT","REL_COUNT"]);
    ...
    switch (unit){
      case 'REL_TIME':{
        ...
        this.once('play', function(){
            ...
            self.stateVars["LastChange"].set("change");
            callback(null);
          });
...
AVTransportService.prototype.processSoap_SetNextAVTransportURI = function(xml, request, callback) {
    ...
    this.stateVars["LastChange"].set("change");
    callback(null);
...
AVTransportService.prototype.lastChange = function(stateVar){
  ...
  var lastJXML = self.device.services[route].lastJXML;
  ...
  if (!lastJXML){
   ...
   self.device.services[route].lastJXML = lastJXML;
  }
