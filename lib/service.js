...
var Service = function(properties, configuration) {
  ...
  this.lastChangeXmlns = properties.lastChangeXmlns;
...
Service.prototype.addAction = function(name, inParameters, outParameters) {
  ...
  if (name.indexOf("Get") === 0 && !this["processSoap_" + name]) {
 ...
 */
Service.prototype.getInstance = function(xml, callback){
  var self = this;
  var route;
...
Service.prototype.processSoapRequest = function(fn, xml, soapAction, request,
  ...
  // scope is this of an instance of service with multiple instances
  this.getInstance(xml, function(soapErrorCode, error, scope){
    ...
    fn.call(scope, xml, request, response, function(soapErrorCode, error) {
      if (error) {
        debug("processControlRequest: Can not process soap action '" +
            soapAction + "': ");
...
Service.prototype.processSoap_Get = function(xml, request, response, callback) {
  ...
  this.stateActions[stateAction].forEach(function(out) {
    ...
    var outVar = self.stateVars[out.type];
    // check type implementation
    if (!outVar){
        logger.error("Cant find out.type :%s in stateVars", out.type);
        return
    }

    _content[out.name] = outVar.get();
    // s-l : handle vars xmlns
    if (outVar.ns) {
      for ( var ns in outVar.ns) {
        xmlns["xmlns:"+ns] = outVar.ns[ns];
      }
    }
  });
