...
Service.prototype.processRequest = function (request, response, path, callback) {
	if (path === this.scpdURL) {
		return this.processScpdRequest(
			request,
			response,
			path,
			function (error) {
...
Service.prototype.processControlRequest = function (request, response, path, callback) {
	...
	logger.debug("Control: soapAction=", soapAction, " headers=", request.headers);
	...
	request.on(
		...
		function () {
			...
			fn.call(
				...
				function (error) {
					if (error) {
						var message = "processControlRequest: Can not process soap action '" +
							soapAction + "': " + error;
						logger.warn(message);
