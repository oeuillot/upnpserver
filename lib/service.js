...
var debugSoap  = require('debug')('upnpserver:service:soap');
...
var Service = function(device, classPrefix, properties, configuration) {
  ...
  // route always relative to description
  this.route   = properties.route;

  this.stateVars = {};

  // store in / out soap variables
  // .get() returns default when null or empty string
  // direct access return null when not found or empty string
  this.soapVars = {
    get:function(prop, _default){
      if (arguments.length < 2) _default = null;
      var _value = this[prop];
  ...
  if (configuration.InstanceID) {
    this.route += "_id_" + configuration.InstanceID;
  }
  ...
  // configId attribute required for upnp device architecture 1.1
  if (this.device.root.ssdp.upnpVersion > 0){
    this._descJXML._attrs["configId"] = this.device.root.ssdp.configId;
  }
...
Service.prototype.pushServiceJXml = function(segment, where) {
  ...
  where.push({
      ...
      _content : {
        ...
        serviceId : this.id,
        SCPDURL : segment + "/" + this.route + "/scpd.xml",
        controlURL : segment + "/" + this.route + "/control",
        eventSubURL : segment + "/" + this.route + "/event"
      }
    });
...
/**
 * define soapVars readOnce accessors
 * so they reset on first get
 */
Service._soapAccessors = function(obj, prop){
  ...
  (function(){
    ...
    Object.defineProperty(this, prop, {
      get : function(){ var val = _value; _value = null; return val; },
...
Service.prototype.addAction = function(name, inParameters, outParameters) {
  ...
  if (inParameters) {
    inParameters.forEach(function(p) {
      ...
      Service._soapAccessors.call(self, self.soapVars, p.name);
...
Service.prototype.addType = function(name, type, errCode, value, valueList, ns,
  evented, moderation_rate, additionalProps, preEventCb, postEventCb) {

  this.stateVars[name] = new StateVar(this, name, type, errCode, value, valueList,
    ns, evented, moderation_rate, additionalProps, preEventCb, postEventCb);
  ...
  this.stateVars[name].pushValueListJXML(r);
  this.stateVars[name].pushRangeJXML(r);
 ...
 * callback(soapErrorCode, error,  scope) scope : the instance
 */
Service.prototype.getServiceInstance = function(xml, callback){
  ...
  if (InstanceID && InstanceID != "0") {
    route = self.route + "_id_" + InstanceID;
  }
...
Service.prototype.processSoapHandlers = function(fn, xml, request, callback){

  var self = this;

  if (!fn) return callback(null);

  fn.call(self, xml, request, function(soapErrorCode, error){
    if (soapErrorCode) {
...
Service.prototype.processSoapInParams = function(fn, xml, soapAction, callback){

  // Process In Params only if we have a handler
  if (!fn) return callback(null);
  var self = this;
    ...
    if (a._content.name === soapAction) {
      ...
      for (j = 0; j < args.length; j++) {
          ...
          if (arg.direction === "in"){
            ...
            this.stateVars[arg.relatedStateVariable].validate(value, function(soapErrorCode, error, value){
              if (error) {
                error = {
              ...
              if (value === ""){
                self.soapVars[arg.name] = null;
              } else {
                self.soapVars[arg.name] = value;
              }
            });
          }
      }
...
Service.prototype.processSoapOutParams = function(fn, xmlns, _content, soapAction, callback){
    ...
    if (a._content.name === soapAction) {
      ...
      for (j = 0; j < args.length; j++) {
          ...
          if (arg.direction === "out"){
            ...
            soapVar  = this.soapVars[arg.name];
            stateVar = this.stateVars[arg.relatedStateVariable];

            if (generic){
              soapVar = stateVar.get();
            }

            if (this.soapVars[arg.name] === undefined){
                throw (new Error("Cant find :"+arg.name+" in soapVars for action"+soapAction));
            }

            _content[arg.name] = soapVar;
            if (stateVar.ns) {
              for ( var ns in stateVar.ns) {
                xmlns[ns] = stateVar.ns[ns];
              }
            }
          }
      }
  ...
  this.getServiceInstance(xml, function(soapErrorCode, error,  scope){
    ...
    Async.series({
      processSoapInParams:function(next){
        self.processSoapInParams.call(scope, fn, xml, soapAction, next);
      },
      processSoapHandlers:function(next){
        self.processSoapHandlers.call(scope, fn, xml, request, next);
      },
      processSoapOutParams:function(next){
        self.processSoapOutParams.call(scope, fn , xmlns, _content, soapAction, next);
      }
    },function(error){
      // soap error response
      if (error){
...
Service.prototype.responseSoap = function(response, functionName, body,
  ...
  debugSoap(functionName + ": Response=", xml);
...
Service.prototype.childrensNamed = function(xml, name, xmlns, list) {
  var self = this;
  if (!list){
    list = [];
  }
  ...
  xml.eachChild(function(c) {
    self.childrensNamed.call(self, c, name, list);
  });

  return list;
};

Service.prototype.childAttributeValue = function(xml, name, xmlns){

  var child = xml.childWithAttribute(name);

  if (child) {
    return child.attr[name].value;
  }

  var found, node;
  xml.eachChild(function(child) {
    node = child.childWithAttribute(name);
    if (node) {
      found = node.attr[name].value;
      return false;
    }
  });
...
Service.prototype.childrensValue = function(xml, name, xmlns) {
  var self = this;
  var list = [];
  var childrens = self.childrensNamed(xml, name, xmlns, list);
  return list.map(function(child){return child.val});
};
