...
var debug = require('debug')('device:service');
var debugEvent = require('debug')('device:service:event');
 ...
 */
Service.prototype.getInstance = function(xml){
  var self = this;
  var route;
  var node = self._childNamed(xml, "InstanceID", Service.UPNP_SERVICE_XMLNS);
  if (node) {
    route = self.route + "_" + node.val;
  }
  if (route && self.device.services[route]){
    return self.device.services[route];
  }
  return this;
}
