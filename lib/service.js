...
/**
 *  A_ARG_TYPE stateVars are only variable type definition, and therefore
 *  some actions does make use of stateVars A_ARG_TYPE twice or more.
 *
 *  In order to still use automatic stateVars in/out mapping,
 *  we need to define internal variable using action name instead of type.
 *
 * eg :
 *  addInternalType("MyVar", "ui4");
 *  addAction([],[{
 *      name:"MyVar",
 *      type:"A_ARG_TYPE_Dup"
 *   }]);
 *
 *  Those variables are not defined in serviceStateTable.
 *  Eventing is disabled by default.
 */
Service.prototype.addInternalType = function(name, type, value, valueList, ns, evented,
    moderation_rate, additionalProps, preEventCb, postEventCb) {

  this.stateVars[name] = new StateVar(this, name, type, value, ns, 0,
      null, additionalProps, preEventCb, postEventCb);
}
    ...
    if (a._content.name === soapAction) {
      ...
      for (j = 0; j < args.length; j++) {
          ...
          if (arg.direction === "in"){
            ...
            if (this.stateVars[arg.name]){
              this.stateVars[arg.name].set(value);
            } else{
    ...
    if (a._content.name === soapAction) {
      ...
      for (j = 0; j < args.length; j++) {
          ...
          if (arg.direction === "out"){
            ...
            if (this.stateVars[arg.name]){
              stateVar = this.stateVars[arg.name];
            } else{
  ...
  this.getServiceInstance(xml, function(soapErrorCode, error,  scope){
    ...
    Async.series({
      ...
      processSoapHandlers:function(next){
        self.processSoapHandlers.call(scope, fn, request, next);
      },
