...
Service.prototype.processControlRequest = function(request, response, path) {
	...
	if (idx > 0) {
		...
		if (type != this.type) {
			console.error("processControlRequest: Invalid type '" + type
					+ "' / '" + this.type + "' !");
			return false;
		}
	...
	if (!fn) {
		console
				.error("processControlRequest: Unknown soap function 'processSoap_"
						+ soapAction + "'");
		return false;
	}
	...
	request.on('end', function() {
		...
		fn.call(self, xml, request, response, function(error) {
			if (error) {
				console.error(
						"processControlRequest: Can not process soap action '"
								+ soapAction + "': ", error);
				return;
			}
			if (log) {
				console.log("Call of soap action '" + soapAction
						+ "': finished");
			}
		});
	});
