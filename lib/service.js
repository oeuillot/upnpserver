...
Service.prototype.processRequest = function(request, response, path, callback) {
	...
	return this.processEventRequest(request, response, path,
		function(error) {
		    callback(error, true);
		});
    }
...
Service.prototype.processUnsubscribe = function(request, response, callback) {
    ...
    if (!sid) {
	return callback("Invalid unsubscribe request sid parameter");
    }
...
Service.prototype.sendEvent = function(serviceVariable, value) {
    ...
    var self = this;
    Async.eachLimit(clientsCopy, CLIENTS_PROCESSOR_LIMIT, function(client,
	    callback) {
	if (client.deleted) {
	    return;
	}
