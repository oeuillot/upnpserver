...
/**
 * define soapArgs accessors
 * so they reset on first get
 */
Service._soapAccessors = function(obj, prop){
  if (obj.hasOwnProperty(prop)) return;
  Object.defineProperty(obj, prop, {
    get : function(){ var val = this.value; this.value = null; return val; },
...
Service.prototype.addAction = function(name, inParameters, outParameters) {
  ...
  if (inParameters) {
    inParameters.forEach(function(p) {
      ...
      Service._soapAccessors(self.soapArgs, p.name);
    ...
    if (a._content.name === soapAction) {
      ...
      for (j = 0; j < args.length; j++) {
          ...
          if (arg.direction === "in"){
            ...
            if (value === ""){
               this.soapArgs[arg.name] = undefined;
            } else {
    ...
    if (a._content.name === soapAction) {
      ...
      for (j = 0; j < args.length; j++) {
          ...
          if (arg.direction === "out"){
            ...
            if (!soapArg){
                throw (new Error("Cant find :"+arg.name+" in soapArgs"));
            }
            _content[arg.name] = soapArgs;
            if (stateVar.ns) {
