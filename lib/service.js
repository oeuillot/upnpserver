...
/**
 * define soapArgs readOnce accessors
...
Service.prototype.addAction = function(name, inParameters, outParameters) {
  ...
  if (inParameters) {
    inParameters.forEach(function(p) {
      ...
      Service._soapAccessors.call(self, self.soapArgs, p.name);
    ...
    if (a._content.name === soapAction) {
      ...
      for (j = 0; j < args.length; j++) {
          ...
          if (arg.direction === "in"){
            ...
            if (value === ""){
              this.soapArgs[arg.name] = null;
            } else {
              this.soapArgs[arg.name] =
    ...
    if (a._content.name === soapAction) {
      ...
      for (j = 0; j < args.length; j++) {
          ...
          if (arg.direction === "out"){
            ...
            soapArg  = this.soapArgs[arg.name];
            stateVar = this.stateVars[arg.relatedStateVariable];

            if (generic){
              soapArg = stateVar.get();
            }

            if (this.soapArgs[arg.name] === undefined){
                throw (new Error("Cant find :"+arg.name+" in soapArgs for action"+soapAction));
            }

            _content[arg.name] = soapArg;
            if (stateVar.ns) {
