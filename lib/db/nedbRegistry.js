...
class NeDbRegistry extends CachedRegistry {
      ...
      this._configureNodesDb(db, (error) => {
        ...
        db2.loadDatabase((error) => {
          ...
          this._configureMetasDb(db2, (error) => {
            ...
            debug("RepositoriesDb path=", path3);
      ...
      super.saveNode(node, modifiedProperties,
          (error) => {
            ...
            if (ASYNC_SAVE) {
              ...
              debug("Async save node #", node.id, "already=", this._saveKeys[node.id]);
  ...
  _dbStore(node, modifiedProperties, callback) {
    ...
    if (modifiedProperties) {
      ...
      debug("dbStore node #", node.id, "storageId=", node.$id, "modifiedProperties=", modifiedProperties, "ms=", ms);
    ...
    this._nodesCollection.update({
      ...
      debug("dbStore: stored node #", node.id, "storageId=", node.$id);
