...
ContentDirectoryService.prototype.addRepository = function(repository, callback) {
  ...
  repository.initialize(this, function(error) {
    ...
    self.repositoriesById[repository.repositoryId] = repository;
  ...
  this.getItemById(objectId, function(error, item) {
    ...
    item.toJXML(repositoryRequest, function(error, itemXML) {
      ...
      if (item.repositoryId) {
        var repository = self.repositoriesById[item.repositoryId];
        if (repository && repository.decorateJXML) {
          repository.decorateJXML(item, itemXML, repositoryRequest, function(
              error, newXML) {
            if (error) {
              return callback(error);
            }

            return produceDidl(item, newXML, callback);
          });
      ...
      return produceDidl(item, itemXML, callback);
    });
  });
};
                            ...
                            item
                                .toJXML(
                                    ...
                                    function(error, itemXML) {
                                      ...
                                      var repository = self.repositoriesById[item.repositoryId];
                                      if (repository && repository.decorateJXML) {
                                        repository.decorateJXML(item, itemXML,
                                            repositoryRequest, function(error,
                                                newXML) {
                                              if (error) {
                                                return callback(error);
                                              }
...
ContentDirectoryService.prototype.newItem = function(parent, name, upnpClass,
    container, attributes, repositoryId, callback) {
  ...
  var item = Item.create(parent, name, upnpClass, container, attributes,
      repositoryId);
...
ContentDirectoryService.prototype.newContainer = function(parent, name,
    upnpClass, attributes, repositoryId, callback) {

  return this.newItem(parent, name, upnpClass || Item.CONTAINER, true,
      attributes, repositoryId, callback);
};
...
ContentDirectoryService.prototype.newFile = function(parent, path, upnpClass,
    stats, attributes, repositoryId, callback) {
  ...
  var self = this;
  self.computeTitle(name, path, mimeType, function(error, title) {
    if (error) {
      return callback(error);
    }
    ...
    attributes.resAttrs.protocolInfo = [ protocol, network, contentFormat,
        additionalInfo ].join(":");

    function waitStats(stats) {
      if (!self.upnpServer.configuration.strict && !attributes.resAttrs.size) {
  ...
  if (path.indexOf(contentPath) === 0) {
    ...
    return this.getItemById(id, function(error, item) {
      ...
      if (item.repositoryId) {
        var repository = self.repositoriesById[item.repositoryId];
        if (repository) {
          return repository.processResponse(item, request, response, path,
              parameters, callback);
        }
      }
