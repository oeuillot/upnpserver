...
//      debugWorker("PreparingNodeWorker node #", node.id, " done error=", error);
  ...
  _emitPrepareNode(node, callback) {
    ...
    this._emitPrepare(mime, contentURL, attributes, callback);
  }

  /**
   * 
   */
  _emitPrepare(mime, contentURL, attributes, callback) {    
    attributes = attributes || {};

    var mainError;

    // console.log("Emit 'prepare:'" + mime+" "+node.attributes.contentURL);
    this.asyncEmit("prepare:" + mime, contentURL, attributes, (error) => {
      if (error === false) {
        // setImmediate(callback);
        // return;
      }
      ...
      this.asyncEmit("prepare:" + mime2, contentURL, attributes, (error) => {
  ...
  loadMetas(infos, callback) {
    ...
    contentProvider.stat(contentURL, (error, stats) => {
      ...
      this.nodeRegistry.getMetas(contentURL, mtime, (error, metas) => {
        debug("getMetas of key=",contentURL,"mtime=",mtime,"=>",metas,"error=",error);
        if (error) {
        ...
        this._emitPrepare(mimeType, contentURL, {}, (error, metas) => {
          if (error) {
