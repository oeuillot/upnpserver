...
ContentDirectoryService.prototype.newContainer = function(parent, name, upnpClass,
		callback) {

	return this.newItem(parent, name, upnpClass || Item.CONTAINER, true, callback);
};

ContentDirectoryService.prototype.newFolder = function(parent, path, stats, upnpClass,
		callback) {
	var name = Path.basename(path);

	return this.newContainer(parent, name, upnpClass, function(error, item, id) {
		if (error) {
			return callback(error);
		}
...
ContentDirectoryService.prototype.newFile = function(parent, path, upnpClass,
	...
	this.newItem(parent, name, upnpClass, false, function(error, item, id) {
		...
		item.path = path;
		item.title=Path.basename(path);
		
		var idx=item.title.indexOf("__");
		if (idx>0) {
			item.title=item.title.substring(0, idx);
		}
