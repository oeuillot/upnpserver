...
var FilterSearchEngine = require('./filterSearchEngine');

var CONTENT_PATH = "/content/";

var PREPARING_QUEUE_CONCURRENCY = 4;
...
var ContentDirectoryService = function(configuration) {
  ...
  this._preparingQueue = Async.priorityQueue(this._preparingNodeWorker
      .bind(this), PREPARING_QUEUE_CONCURRENCY);
...
ContentDirectoryService.prototype.initialize = function(upnpServer, callback) {
  ...
  Service.prototype.initialize.call(this, upnpServer, (error) => {
    if (error) {
...
ContentDirectoryService.prototype.allocateNodesForPath = function(path,
    callback) {
...
ContentDirectoryService.prototype.processBrowseMetadata = function(response,
    request, objectId, filterSearchEngine, callback) {
...
ContentDirectoryService.prototype.getNodeJXML = function(node,
  ...
  this.prepareNodeAttributes(node, ContentDirectoryService.SYNC_PRIORITY, (error, node) => {
    ...
    itemClass.toJXML(node, attributes, repositoryRequest, filterCallback, (error, itemJXML) => {
      ...
      this.emitToJXML(node, attributes, repositoryRequest,
...
ContentDirectoryService.prototype.processBrowseDirectChildren = function(
    response, request, containerId, filterSearchEngine, startingIndex,
...
ContentDirectoryService.prototype.browseNode = function(node, callback) {
  ...
  this.asyncEmit("browse", list, node, function(error) {
    ...
    debug("browseNode #", node.id, "path=", path, "returns=", list.length,
    "elements.");
...
ContentDirectoryService.prototype.createNode = function(name, upnpClass,
  ...
  Node.create(this, name, upnpClass, attributes, (error, node) => {
    ...
    upnpClass.prepareNode(node, (error) => {
      if (error) {
        return callback(error);
      }
...
ContentDirectoryService.prototype.newNode = function(parentNode, name,
  ...
  this.createNode(name, upnpClass, attributes, (error, node) => {
    if (error) {
      logger.error("Can not create node #", node.id, "error=", error);
      return callback(error);
    }
...
ContentDirectoryService.prototype.refreshNode = function(node, callback) {
  ...
  this.asyncEmit("update", node, (error) => {
    if (error) {
      logger.error("CDS: refreshNode '" + path + "' returns error ", error);
      return callback(error);
    }
...
ContentDirectoryService.prototype.saveNode = function(node, modifiedProperties,
    callback) {
...
ContentDirectoryService.prototype.allocateNodeId = function(callback) {
...
ContentDirectoryService.prototype.prepareNodeAttributes = function(node,
  ...
  if (refID) {
    ...
    node.resolveLink((error, refNode) => {
      ...
      this.prepareNodeAttributes(refNode, options, function(error) {
        if (error) {
          return callback(error);
        }
...
//  debug("PreparingNodeWorker: preparing... #",node.id,"prepared=",node.prepared);
    ...
    this.emitPrepareNode(node, (error) => {
...
ContentDirectoryService.prototype.searchUpnpClass = function(fileInfos) {
  var list = [];
  ...
  if (fileInfos.mime) {
    ...
    if (upnpClasses) {
      upnpClasses.forEach(function(upnpClass) {
        list.push({
          upnpClass : upnpClass,
          priority : 20
        });
  ...
  if (false) {
    console.log("Return ", Util.inspect(list, {
      depth : null
    }));
  }
