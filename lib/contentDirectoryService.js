...
ContentDirectoryService.prototype.prepareNodeAttributes = function(node,
  ...
  if (callback) {
    callback = _stackCallback(node, callback);
  }

  this._preparingQueue.push(node, priority, callback);
};

function _stackCallback(node, callback) {
  ...
  return function(error) {
    ...
    setImmediate(callback.bind(self, error, node));
  };
}
...
ContentDirectoryService.prototype.emitPrepareNode = function(node, callback) {
  ...
  this.asyncEmit("prepare:" + mime, node, function(error) {
    ...
    self.asyncEmit("prepare:" + mime2, node, callback);
  });
};

ContentDirectoryService.prototype.emitToJXML = function(node, request, xml,
    callback) {
  ...
  var self = this;
  this.asyncEmit("toJXML:" + mime, node, request, xml, function(error) {
    if (error === false) {
      return callback();
    }
    ...
    self.asyncEmit("toJXML:" + mime2, node, request, xml, function(error) {
      if (error !== false) {
        return callback(error);
      }
