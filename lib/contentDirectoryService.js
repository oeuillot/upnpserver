...
const debugWorker = debugFactory('upnpserver:contentDirectoryService:worker');
...
class ContentDirectoryService extends Service {
  constructor(configuration) {
    ...
    this._childrenByContentURLWeakHashmap = new NodeWeakHashmap("childrenListByContentURL", 5000, true);
...
// kept for Intel upnp toolkit, but not in upnp spec
  ...
  prepareNodeAttributes(node, options, callback) {
    ...
    if (debugStack.enabled) {
      debugStack("prepareNodeAttributes", "prepare node=#", node.id, "depth=", _stackDepth());
    }
    ...
    if (callback) {
      ...
      callback = (error) => {
        ...
        if (false && debugWorker.enabled) {
          debugWorker("prepareNodeAttributes", "node #", node.id, " done error=", error);
        }
...
// debug("PreparingNodeWorker: preparing... #",node.id,"prepared=",node.prepared);
      ...
      if (!node.contentURL) {
        this.saveNode(node, { $unset: { prepared: true } }, (error) => {
          node.leaveLock("preparing");
      ...
      this.loadMetas({ mime: nodeAttributes.mime, contentURL: node.contentURL }, (error, attributes) => {
// debugWorker("PreparingNodeWorker node #", node.id, " done error=", error);
        ...
        for(var k in attributes) {
          nodeAttributes[k]=attributes[k];
        }

        this.saveNode(node, { attributes: nodeAttributes, $unset: { prepared: true } }, (error) => {
          node.leaveLock("preparing");
