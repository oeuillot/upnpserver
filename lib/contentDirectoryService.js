...
var ContentDirectoryService = function(device, classPrefix, configuration) {
  ...
  this.addInternalType("NumberReturned", "ui4", 0);
  this.addInternalType("TotalMatches", "ui4", 0);
...
ContentDirectoryService.prototype.processSoap_Search = function(request, callback) {

  var self = this;
  var objectId = this.root.id;
  var node = self.stateVars["A_ARG_TYPE_ObjectID"].get();
  if (node) {
    objectId = node;
  }

  var searchCriteria = null;
  node = self.stateVars["A_ARG_TYPE_SearchCriteria"].get();
  if (node) {
    searchCriteria = node;
  }

  var filterCallback = null;
  node = self.stateVars["A_ARG_TYPE_Filter"].get();
  if (node) {
    // var fs = node;
    ...
    filterCallback = this._prepareFilterCallback(node, {});

  }

  var startingIndex = -1;
  node = self.stateVars["A_ARG_TYPE_Index"].get();
  if (node) {
    startingIndex = parseInt(node, 10);
  }

  var requestedCount = -1;
  node = self.stateVars["A_ARG_TYPE_Count"].get();
  if (node) {
    requestedCount = parseInt(node, 10);
  }

  var sortCriteria = null;
  node = self.stateVars["A_ARG_TYPE_SortCriteria"].get();
  if (node) {
    sortCriteria = node;
  }
            ...
            function processList(list, node) {
              ...
              Async
                ...
                // and Markup) before embedding in a SOAP response message
                        ...
                        self.stateVars["A_ARG_TYPE_Result"].set(didl);
                        self.stateVars["NumberReturned"].set(lxml.length);
                        self.stateVars["TotalMatches"].set(total);
                        self.stateVars["A_ARG_TYPE_UpdateID"].set(updateId);
...
ContentDirectoryService.prototype.processSoap_Browse = function(request, callback) {
  ...
  var browseFlag = null;
  node = self.stateVars["A_ARG_TYPE_BrowseFlag"].get();
  if (node) {
    browseFlag = node;
  }
  ...
  if (browseFlag === "BrowseMetadata") {
    return this.processBrowseMetadata(objectId,
...
ContentDirectoryService.prototype.processBrowseMetadata = function(
  ...
  // and Markup) before embedding in a SOAP response message
              ...
              self.stateVars["A_ARG_TYPE_Result"].set(didl);
              self.stateVars["NumberReturned"].set(1);
              self.stateVars["TotalMatches"].set(1);
              self.stateVars["A_ARG_TYPE_UpdateID"].set(updateId);
              callback(null);
          }
