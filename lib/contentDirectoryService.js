 ...
 */
ContentDirectoryService.prototype.lastChange = function(action, node){
  ...
  for (var i=0; i<len; i++){
    if (stateEvent[i]._name == action){
      stateAction = stateEvent[i]._content;
      break;
    }
  }
  if (!stateAction){
    ...
    stateAction = newstateAction._content;
    stateEvent.push(newstateAction)
  }
  // find node or create one
  var len = stateAction.length;
  var Item;
  for (var i=0; i<len; i++){
    if (stateAction[i]._objID == node.id){
      Item = stateAction[i];
    break;
    }
  }
  if (!Item){
    ...
    stateAction.push(Item);
  }
  switch (action){
    case "objAdd":{
      Item["objParentID"] = node.parentId;
      Item["objClass"]    = node.upnpClass;
    }
    case "objMod":
    case "objDel":
      Item["stUpdate"] = true;
