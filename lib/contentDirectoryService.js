	...
	Service.prototype.initialize.call(this, upnpServer, function(error) {
		...
		self.newItem(null, "$root", "$root", function(error, item) {
			...
			Async.eachSeries(repositories, function(repository, callback) {
				...
				console.log("Adding repository", repository);
	...
	Async.reduce(ps, this.root, function(parentItem, segment, callback) {
		...
		parentItem.getChildByName(segment, function(error, item) {
			...
			if (item) {
				...
				console.log("allocateItemsForPath(" + segment + ")=>", item);
				return callback(null, item);
			}

			console
					.log("allocateItemsForPath(" + segment
							+ ")=> NEW CONTAINER");
...
ContentDirectoryService.prototype.processSoap_Browse = function(xml, request,
	...
	this.upnpServer.getItemById(containerId, function(error, item) {
		console.log("CDS: Browser item=", item, " error=", error);
		...
		item.listChildren(function(error, list) {
			...
			console.log("CDS: List itemId=", item.itemId, " path=", item.path,
					" error=", error, " list=", list.length, " startingIndex=",
					startingIndex, " requesteCount=", requestedCount);
			...
			/*
			 * 
			 * var didl = "<DIDL-Lite >" + " <item id=\"16.0_0\" parentID=\"0\"
			 * restricted=\"1\">" + " <dc:title>100_0564.JPG</dc:title>" + "
			 * <res protocolInfo=\"http-get:*:image/jpeg:*\"
			 * resolution=\"500x500\">http://192.168.0.199:41955/pictures/16.0_0.JPG</res>" + "
			 * <upnp:class>object.item.imageItem.photo</upnp:class>" + "
			 * </item>" + " <item id=\"16.0_1\" parentID=\"0\"
			 * restricted=\"1\">" + " <dc:title>100_0565.JPG</dc:title>" + "
			 * <res protocolInfo=\"http-get:*:image/jpeg:*\"
			 * resolution=\"500x500\">http://192.168.0.199:41955/pictures/16.0_1.JPG</res>" + "
			 * <upnp:class>object.item.imageItem.photo</upnp:class>" + "
			 * </item>" + "</DIDL-Lite>";
			 */
...
ContentDirectoryService.prototype.browseItem = function(item, callback) {
	...
	Async.reduce(this.repositories, [], function(list, repository, callback) {
		...
		if (path.indexOf(repository.mountPath)) {
			console.log("Not in mountpath for repository mountPath="
					+ repository.mountPath);
			return callback(null, list);
		}

		console.log("Ask repository browse mountPath=" + repository.mountPath);
		...
		repository.browse(list, item, function(error, result) {
			...
			if (!result || !result.length) {
				...
				console.log("Ask " + repository.mountPath
						+ " => No result list=" + list.length);

				return callback(null, list);
			}

			console.log("Ask " + repository.mountPath + " => " + result.length,
					" list=" + list.length);
