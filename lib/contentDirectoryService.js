...
var ContentDirectoryService = function(device, classPrefix, configuration) {
  ...
  this.addType("A_ARG_TYPE_BrowseFlag", "string", 600, "", [ "BrowseMetadata",
      "BrowseDirectChildren" ]);
  this.addType("ContainerUpdateIDs", "string", 600, 0, [], null, true, 2, [],
  ...
  this.addType("SystemUpdateID", "ui4", 600, 0, [], {
    "xmlns:dt" : Xmlns.MICROSOFT_DATATYPES,
    "dt:dt" : "int"
  }, true, 2);

  this.addType("A_ARG_TYPE_Count", "ui4", 600, 0);

  this.addType("A_ARG_TYPE_SortCriteria", 709, "string", "");
  this.addType("A_ARG_TYPE_SearchCriteria", 708, "string", "");

  this.addType("SortCapabilities", "string", 600, [ "dc:title", "upnp:genre",
      "upnp:artist", "upnp:author", "upnp:album", "upnp:rating" ].join(','),
      [], {
        "xmlns:upnp" : Xmlns.UPNP_METADATA,
        "xmlns:dc" : Xmlns.PURL_ELEMENT
      });
  this.addType("A_ARG_TYPE_Index", "ui4", 600, 0);
  this.addType("A_ARG_TYPE_ObjectID", "string", 710);
  this.addType("A_ARG_TYPE_UpdateID", "ui4", 600, 0);
  this.addType("A_ARG_TYPE_Result", "string", 600);
  this.addType("SearchCapabilities", "string", 600, [ "dc:title", "upnp:genre",
      "upnp:artist", "upnp:author", "upnp:album", "upnp:rating" ].join(','),
      [], {
        "xmlns:upnp" : Xmlns.UPNP_METADATA,
        "xmlns:dc" : Xmlns.PURL_ELEMENT
      });
  this.addType("A_ARG_TYPE_Filter", "string", 600);
  ...
  if (this.version > 2){
    ...
    // http://upnp.org/specs/av/UPnP-av-AVDataStructureTemplate-v1.pdf table 3
      this.addType("LastChange", "string", 600, "", [], {"cds-event" : "urn:schemas-upnp-org:av:cds-event"}, true, 0.2);
  }
  ...
  this.updateIds = {};
  this.contentPath = "/" + this.device.route + "/" + this.route + "/content/";

  this._preparingQueue = Async.priorityQueue(this._preparingNodeWorker
      .bind(this), PREPARING_QUEUE_LIMIT);

  this._fileContentProvider = new FileContentProvider(this, configuration);
...
ContentDirectoryService.prototype.initialize = function(callback) {

  this.dlnaSupport = this.device.dlnaSupport;


  var self = this;
  Service.prototype.initialize.call(this, function(error) {
    if (error) {
    ...
    function(error){
      ...
      if (self.device.configuration.enableIntelToolkitSupport){
        self._intervalTimer = setInterval(function() {
          self._sendItemChangesEvent();
        }, 1500);
      }
      if (self.device.configuration.garbageItems) {
...
ContentDirectoryService.prototype.processSoap_Search = function(xml, request,
  ...
  this.processSoap_Browse(xml, request, callback);
};
...
ContentDirectoryService.prototype._newDidlJxml = function() {
  ...
  if (this.jasminMovieMetadatasSupport) {
    attrs["xmlns:mmov"] = Xmlns.JASMIN_MOVIEMETADATA;
  }
...
ContentDirectoryService.prototype.processSoap_Browse = function(xml, request, callback) {

    var self = this;

    // use .get() to set default when null
    var objectId        = self.soapVars.get("ObjectID",  this.root.id);
      // support for search action
      objectId          = self.soapVars.get("ContainerID",  objectId);

    var searchCriteria  = self.soapVars["SearchCriteria"];
    var startingIndex   = self.soapVars.get("StartingIndex",  -1);
    var requestedCount  = self.soapVars.get("RequestedCount", -1);
    var sortCriteria    = self.soapVars["SortCriteria"];
    // set to directChildren by default for search requests
    var browseFlag      = self.soapVars.get("BrowseFlag", "BrowseDirectChildren");

    var searchNode = Service._childNamed(xml, "SearchCriteria", Xmlns.UPNP_SERVICE);
    var filterNode = Service._childNamed(xml, "Filter", Xmlns.UPNP_SERVICE);
  ...
  if (browseFlag === "BrowseMetadata") {
    ...
    return this.processBrowseMetadata(request, objectId,
        filterSearchEngine, callback);
  }

  if (browseFlag === "BrowseDirectChildren") {
    return this.processBrowseDirectChildren(request, objectId,
        filterSearchEngine, startingIndex, requestedCount, sortCriteria, !!searchNode, callback);
  }

  callback("Unknown browseFlag '" + browseFlag + "'");
};

ContentDirectoryService.prototype.processBrowseMetadata = function(
      ...
      .getNodeById(
          ...
          function(error, node) {
            ...
            function produceDidl(node, nodeXML) {
              ...
              var updateId = (node.id) ? node.updateId :
                  self.stateVars["SystemUpdateID"].get()
              self.soapVars["Result"] = didl;
              self.soapVars["NumberReturned"] = 1;
              self.soapVars["TotalMatches"] = 1;
              self.soapVars["UpdateID"] = updateId;
              callback(null);
          }

            filterSearchEngine.start(node);

            self.getNodeJXML(node, null, repositoryRequest, filterSearchEngine.func,
...
ContentDirectoryService.prototype.processBrowseDirectChildren = function(
  request, containerId, filterSearchEngine, startingIndex,
                  ...
                  .eachSeries(
                      ...
                      function(child, callback) {
                        ...
                        self.getNodeJXML(child, null, repositoryRequest,
                            filterSearchEngine.func, function(error, itemJXML) {
                              if (error) {
                                return callback(error);
                              }
                      ...
                      function(error) {
                        ...
                        var updateId = (node.id) ? node.updateId :
                            self.stateVars["SystemUpdateID"].get()
                        self.soapVars["Result"] = didl;
                        self.soapVars["NumberReturned"] = lxml.length;
                        self.soapVars["TotalMatches"] = total;
                        self.soapVars["UpdateID"] = updateId;
