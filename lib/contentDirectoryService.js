...
function escapeXml(xml) {
    // VLC does not support CDATA form !
...
var ContentDirectoryService = function() {
    ...
    this._childrenWeakHashmap=new ItemWeakHashmap(100);
    this.repositories = [];
...
ContentDirectoryService.prototype.initialize = function(upnpServer, callback) {
	...
	return self.setRepositories(repositories, callback);
    });
};
...
ContentDirectoryService.prototype.responseObject = function(response, request,
    ...
    this
	    .getItemById(
			...
			logger.info("CDS: BrowseObject itemId=", item.id,
				" error=", error);
...
ContentDirectoryService.prototype.responseContainer = function(response,
    ...
    this
	    .getItemById(
			...
			if (error) {
			    logger.error("CDS: Can not getItemById for id",
				    containerId);
			    return callback(error);
			}
			if (!item) {
			    return callback("CDS: Browser Can not find item "
				    + containerId);
			}

			logger.debug("CDS: Browser itemId=", item.id,
				" error=", error);
			...
			item
				.listChildren(function(error, list) {
				    if (error) {
					logger.warn(
						"Can not scan repositories: ",
						error);
					return callback(error);
				    }

				    // if (false) {
				    // logger.debug("CDS: List itemId=",
				    // item.id, " path=",
				    // item.path, " error=", error, " list=",
				    // list.length,
					...
					if (!item || !item.toJXML) {
					    logger.warn("ALERT not an item ",
						    item);
					    return;
					}

					lxml.push(item
						.toJXML(repositoryRequest));
				    });
...
ContentDirectoryService.prototype.browseItem = function(item, callback) {
	...
	if (path.indexOf(repository.mountPath) !== 0) {
	    ...
	    logger.debug("CDS: browseItem repository mountPath="
		    + repository.mountPath + " path=" + path
		    + " is not in mountpath");
	...
	if (error) {
	    logger
		    .error("CDS: browseItem '" + path + "' returns error ",
			    error);
	    return callback(error);
	}
...
ContentDirectoryService.prototype.updateItem = function(item, callback) {
	...
	if (error) {
	    logger
		    .error("CDS: updateItem '" + path + "' returns error ",
			    error);
	    return callback(error);
	}
