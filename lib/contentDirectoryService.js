...
const debugWorker = debugFactory('upnpserver:worker');
...
const xmlFilters = require("./xmlFilters").xmlFilters;
...
const GARBAGE_DELAY_MS = 1000 * 6; // 60 * 60;
...
class ContentDirectoryService extends Service {
  constructor(configuration) {
    ...
    this.jasminMovieMetadatasSupport = (configuration.jasminMovieMetadatasSupport === true);
    ...
    if (process.env.JASMIN) {
      ...
      this.jasminMovieMetadatasSupport = true;
    }
    ...
    this._preparingQueue = Async.priorityQueue((node, callback) => this._preparingNodeWorker(node, callback), PREPARING_QUEUE_CONCURRENCY);
  ...
  initialize(upnpServer, callback) {
    ...
    super.initialize(upnpServer, (error) => {
      ...
      Async.eachSeries(this.contentHandlers, (contentHandler, callback) => {
        ...
        this.contentHandlersById[contentHandler.key] = contentHandler;
        ...
        this._installRoot((error, root) => {
          ...
          return this.addRepositories(repositories, (error) => {
            if (error) {
              return callback(error);
            }
            ...
            if (upnpServer.configuration.garbageItems) {
              ...
              this._intervalGarbage = setInterval(() => this._garbageItems(), Math.floor(GARBAGE_DELAY_MS / 10));
   ...
   * 
   */
  allocateNodesForPath(path, callback) {

    debug("allocateNodesForPath path=", path);
    ...
    Async.reduce(ps, this.root, (parentNode, segment, callback) => {

      parentNode.getChildByName(segment, (error, node) => {
        if (error) {
          return callback(error);
        }
        ...
        this.newNode(parentNode, segment, UpnpContainer.UPNP_CLASS, {
          virtual : true
    ...
    this.getNodeById(containerId, (error, item) => {
      ...
      var processList = (list, node) => {
        ...
        Async.eachSeries(list, (child, callback) => {
          ...
          this.getNodeJXML(child, null, repositoryRequest,
      ...
      if (item.refID) {
        this.getNodeById(item.refID, (error, refItem) => {
          ...
          refItem.filterChildNodes(refItem, null, filter, (error, list) => {
            if (error) {
      ...
      debug("Browser itemId=#", item.id, "error=", error);

      item.filterChildNodes(item, null, filter, (error, list) => {
        if (error) {
          logger.warn("Can not scan repositories: ", error);
          return callback(710, error);
        }
  ...
  processBrowseMetadata(response, request, objectId, filterSearchEngine, callback) {
    ...
    this.getNodeById(objectId, (error, node) => {
      ...
      this.getNodeJXML(node, null, repositoryRequest,
  ...
  getNodeJXML(node, inheritedAttributes, repositoryRequest, filterCallback, callback) {
    ...
    if (refID) {
      node.resolveLink((error, refNode) => {
        ...
        this.getNodeJXML(refNode, linkAttributes, repositoryRequest,
    ...
    this.prepareNodeAttributes(node, ContentDirectoryService.SYNC_PRIORITY, (error, node) => {
      if (error) {
        return callback(error);
      }
  ...
  processBrowseDirectChildren(response, request, containerId, filterSearchEngine, startingIndex,
    ...
    this.getNodeById(containerId, (error, item) => {
      ...
      this.emit("BrowseDirectChildren", request, item);
      ...
      var processList = (list, node) => {
        ...
        Async.eachSeries(list, (child, callback) => {
          ...
          this.getNodeJXML(child, null, repositoryRequest,
              filterSearchEngine.func, (error, itemJXML) => {
                if (error) {
                  return callback(error);
                }

                itemJXML = filterSearchEngine.end(itemJXML);
                if (itemJXML) {
                  lxml.push(itemJXML);
                }
                setImmediate(callback);
      ...
      if (item.refID) {
        this.getNodeById(item.refID, (error, refItem) => {
          ...
          refItem.listChildren((error, list) => {
            if (error) {
              logger.warn("Can not scan repositories: ", error);
              return callback(501, error);
            }
  ...
  browseNode(node, callback) {
    ...
    this.asyncEmit("browse", list, node, (error) => {
      if (error) {
   ...
   * 
   */
  createNodeRef(targetNode, name, callback) {
    ...
    Node.createRef(targetNode, name, callback);
  }

  /**
   * 
   */
  createNode(name, upnpClass, attributes, prepared, initCallback, callback) {

    // assert(!attributes, "Invalid attributes parameter"); // It can be undefined ! (link)
    assert(upnpClass, "Invalid upnpClass parameter");
    assert(initCallback===undefined || initCallback===null || typeof(initCallback)==="function", "Invalid initCallback parameter");
   ...
   * 
   */
  newNodeRef(parent, targetNode, name, before, callback) {
    ...
    this.createNodeRef(targetNode, name, (error, node) => {
      if (error) {
   ...
   * 
   */
  newNode(parentNode, name, upnpClass, attributes, prepared, initCallback, before, callback) {
    ...
    this.createNode(name, upnpClass, attributes, prepared, initCallback, (error, node) => {
      if (error) {
  ...
  refreshNode(node, callback) {
    // Il faut identifier le repository associé à cet item

    var path = node.path;
    debug("refreshNode #", node.id, " path=", path, "repositories.count=",
        this.repositories.length);

    this.asyncEmit("update", node, (error) => {
      if (error) {
        logger.error("CDS: refreshNode #"+node.id+" path=" + path + " returns error ", error);
        return callback(error);
      }

      debug("refreshNode #", node.id, " path=", path, "done");

      // C'est un refresh pas un SAVE !
      this.saveNode(node, callback);
    });
  }
  ...
  allocateNodeId(callback) {
    this._nodeRegistry.allocateNodeId(callback);
  }
  ...
  processRequest(request, response, path, callback) {
    ...
    case "content":
      ...
      var id = this._nodeRegistry.keyFromString(action);
      ...
      return this.getNodeById(id, (error, node) => {
        if (error) {
        ...
        node.resolveLink((error, nodeRef) => {
          ...
          this.processNodeContent(nodeRef, request, response, path, parameters,
              callback);
        });
      });

    case "tree":
      return this.getNodeById(0, (error, node) => {
        if (error) {
  ...
  _garbageItems() {
    ...
    this.root.garbage((error) => {

      debugGarbage("Garbage done !");

      this._garbaging = false;
    });
  }
...
//kept for Intel upnp toolkit, but not in upnp spec
  ...
  prepareNodeAttributes(node, options, callback) {

    if (debugStack.enabled) {
      debugStack("prepareNodeAttributes node=#", node.id, "depth=", _stackDepth());
    }
    ...
    if (refID) {
      ...
      node.resolveLink((error, refNode) => {
        ...
        this.prepareNodeAttributes(refNode, options, (error) => {
          if (error) {
            return callback(error);
          }
    ...
    if (debugWorker.enabled) {
      debugWorker("PrepareNodeAttributes node #", node.id + "prepared=",
          node.prepared, "callback=" + (!!callback), "priority=", priority);
    }
    ...
    if (callback) {
      ...
      callback = (error) => {
        ...
        setImmediate(callbackOld.bind(this, error, node));
      };
    }
   ...
   * 
   */
  _preparingNodeWorker(node, callback) {
    assert(typeof(node)==="object", "Node is null");

//  debug("PreparingNodeWorker: take semaphore");

    node._take("preparing", () => {

//    debug("PreparingNodeWorker: preparing... #",node.id,"prepared=",node.prepared);

      if (node.prepared !== false) {
        node._leave("preparing");
        return callback();
      }

      delete node.prepared;

      debugWorker("PreparingNodeWorker node #", node.id);

      if (debugStack.enabled) {
        debugStack("PreparingNodeWorker node=#", node.id, "depth=", _stackDepth());
      }

      this._emitPrepareNode(node, (error) => {
//      debugWorker("PreparingNodeWorker node #", node.id, " done error=", error);

        this.saveNode(node, null, (error, newNode) => {
          node._leave("preparing");
   ...
   * 
   */
  _emitPrepareNode(node, callback) {
    ...
    this.asyncEmit("prepare:" + mime, node, (error) => {
      if (error === false) {
        // setImmediate(callback);
        // return;
      }
      ...
      this.asyncEmit("prepare:" + mime2, node, (error) => callback(error || mainError));
    });
  }
  ...
  searchUpnpClass(fileInfos) {
    ...
    if (contentHandlers) {
      for (var i = 0; i < contentHandlers.length; i++) {
        var contentHandler = contentHandlers[i];
    ...
    if (false) {
      debug("Return ", Util.inspect(list, {
        depth : null
      }));
    }
  ...
  loadMetas(infos, callback) {
    ...
    var ar=contentHandlers.filter((contentHandler) => mimeTypeMatch(mimeType, contentHandler.mimeTypes));
    ar.sort((ch1, ch2) => {
...
//    console.log("ContentHandlers=",ar);

    Async.eachSeries(ar, (contentHandler, callback) => {

      contentHandler.prepareMetasFromContentURL(infos.contentURL, metas, callback);

    }, (error) => {
      if (error) {
        logger.error("Can not load metas of ",infos, error);
        return callback(error);
      }

      debug("loadMetas from contentHandler return ", metas);

      callback(error, metas);
    });
  }
}
...
var ACCENTS_MAPPER = [ /[áãàâäåāăąǎǟǡǻ]/g, 'a', /[çćĉċč]/g, 'c', /[ďđ]/g, 'd',
