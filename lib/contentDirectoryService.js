...
(function() {
  ...
  var classes = [ "object.item", "object.container", "object.item.audioItem",
      "object.item.musicTrack", "object.item.videoItem",
  ...
  classes.forEach(function(clazz) {
    ...
    itemClasses[Clz.upnpClass] = new Clz();
  });
})();
...
ContentDirectoryService.prototype.responseObject = function(response, request,
  ...
  this.getNodeById(objectId, function(error, item) {
    ...
    var repositoryRequest = {
      ...
      request : request,
      dnlaSupport : self.upnpServer.dlnaSupport,
      upnpServer : self.upnpServer
    };
    ...
    itemClass.toJXML(item, repositoryRequest, function(error, itemXML) {
      if (error) {
        return callback(error);
      }
                ...
                .listChildren(function(error, list) {
                  ...
                  var repositoryRequest = {
                    contentURL : "http://" + localhost + ":" + localport +
                        self.contentPath,
                    request : request,
                    dlnaSupport : self.upnpServer.dlnaSupport,
                    upnpServer : self.upnpServer
                  };

                  if (repositoryRequest.dnlaSupport) {
                    xmlDidl._attrs["xmlns:dlna"] = "urn:schemas-dlna-org:metadata-1-0/";
                  }
                  ...
                  Async
                      .eachSeries(
                          ...
                          function(item, callback) {
                            ...
                            var itemClass = ContentDirectoryService._itemClasses[item.upnpClass];
                            assert(itemClass, "Item class is not defined for " +
                                item.upnpClass);

                            itemClass.toJXML(item, repositoryRequest, function(
                                error, itemXML) {
                              if (error) {
                                return callback(error);
                              }
...
ContentDirectoryService.prototype.newItem = function(parent, name, upnpClass,
  ...
  var self = this;
  return itemClass.init(parent, name, upnpClass, container, attributes,
      function(error, name, attributes) {
        if (error) {
          return callback(error);
        }

        var item = Node.create(parent, name, upnpClass, container, attributes);

        self.registerItem(item, function(error) {
          if (error) {
...
ContentDirectoryService.prototype.registerItem = function(item, callback) {
  this._nodeRegistry.registerItem(item, callback);
};
...
ContentDirectoryService.prototype.processRequest = function(request, response,
  ...
  if (path.indexOf(contentPath) === 0) {
    ...
    return this.getNodeById(id, function(error, item) {
      ...
      return itemClass.processRequest(item, request, response, path,
          parameters, callback);
    });
  }
