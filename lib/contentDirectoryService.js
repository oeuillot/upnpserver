...
var JASMIN_FILEMETADATA_XMLNS = "urn:schemas-jasmin-upnp.net:filemetadata/";
var JASMIN_MUSICMETADATA_XMLNS = "urn:schemas-jasmin-upnp.net:musicmetadata/";
...
ContentDirectoryService.prototype.processSoap_Search = function(xml, request,
  ...
  function childNamed(name, xmlns) {
    var node = Service._childNamed(xml, name);
    return node;
  }

  var objectId = this.root.id;
  node = childNamed("ContainerID");
  if (node) {
    objectId = node.val;
  }

  var searchCriteria = null;
  var node = childNamed("SearchCriteria");
  if (node) {
...
ContentDirectoryService.prototype.processSoap_Browse = function(xml, request,
  ...
  var searchCriteria = null;
  var node = childNamed("SearchCriteria", UPNP_SERVICE_XMLNS);
  if (node) {
    searchCriteria = node.val;
  }

  var browseFlag = null;
  node = childNamed("BrowseFlag", UPNP_SERVICE_XMLNS);
  if (node) {
    browseFlag = node.val;
  }

  var attributesFilter = null;
  node = childNamed("Filter", UPNP_SERVICE_XMLNS);
  if (node && node.val) {
    var fs = node.val;
  ...
  var objectId = this.root.id;
  node = childNamed("ObjectID", UPNP_SERVICE_XMLNS);
  if (node) {
  ...
  var startingIndex = -1;
  node = childNamed("StartingIndex", UPNP_SERVICE_XMLNS);
  if (node) {
    startingIndex = parseInt(node.val, 10);
  }

  var requestedCount = -1;
  node = childNamed("RequestedCount", UPNP_SERVICE_XMLNS);
  if (node) {
    requestedCount = parseInt(node.val, 10);
  }

  var sortCriteria = null;
  node = childNamed("SortCriteria", UPNP_SERVICE_XMLNS);
  if (node) {
    sortCriteria = node.val;
  }
