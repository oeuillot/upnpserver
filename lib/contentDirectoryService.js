...
var PathRepository = require('./repositories/pathRepository');
var MusicRepository = require('./repositories/musicRepository');
var HistoryRepository = require('./repositories/historyRepository');
var IceCastRepository = require('./repositories/iceCastRepository');
...
var ContentDirectoryService = function(configuration) {
  ...
  this.addType("SystemUpdateID", "ui4", 0, [], {
    dt : Service.MICROSOFT_DATATYPES_XMLNS
  }, true, 2);
  ...
  this.addType("SortCapabilities", "string", [ "dc:title", "upnp:genre",
      ...
      [], {
        upnp : Service.UPNP_METADATA_XMLNS,
  ...
  this.jasminFileMetadatasExtension = (configuration.jasminFileMetadatasExtension === true);
  this.jasminMusicMetadatasExtension = (configuration.jasminMusicMetadatasExtension === true);

  if (process.env.JASMIN) {
    // Will be removed soon :-)
    this.jasminFileMetadatasExtension = true;
    this.jasminMusicMetadatasExtension = true;
...
ContentDirectoryService.UPNP_METADATA_XMLNS = "urn:schemas-upnp-org:metadata-1-0/upnp/";;
ContentDirectoryService.DLNA_METADATA_XMLNS = "urn:schemas-dlna-org:metadata-1-0/";
...
ContentDirectoryService.prototype.initUpnpClass = function(callback){
  ...
  fs.readdir(__dirname + "/class/", function(err, upnpClassFiles){
    if (err) return callback(err);
    upnpClassFiles.forEach(function(file){
      if (/^object.*/.test(file)){
        var upnpClassName = file.substr(0, file.length-3);
        var clazz = require("./class/" + upnpClassName);
        self.upnpClasses[upnpClassName] = new clazz();
      }
    });
    callback(null, true);
  });
...
ContentDirectoryService.prototype.initContentHandlers = function(callback){
  ...
  fs.readdir(__dirname + "/contentHandlers/", function(err, contentHandlerFiles){
    if (err) return callback(err);
    Async.eachSeries(contentHandlerFiles, function(file, callback){
      if (/^contentHandler_/.test(file)){
        var contentHandler = file.substr(0, file.length-3);
        var clazz = require("./contentHandlers/" + contentHandler);
        var ch = new clazz();
        self.contentHandlers.push(ch);
        self.contentHandlersById[ch.key] = ch;
        ch.initialize(self, callback);
      }
      else callback(null);
    }, function(err){
      callback(err);
 ...
 * @param path
 */
ContentDirectoryService.prototype.initPaths = function(path) {
 ...
 *            path
 */
ContentDirectoryService.prototype.addDirectory = function(mountPoint, path) {
 ...
 *            repository
 */
ContentDirectoryService.prototype.addRepository = function(repository) {
 ...
 *            path
 */
ContentDirectoryService.prototype.addMusicDirectory = function(mountPoint, path) {
  assert(typeof mountPoint === "string", "Invalid mountPoint parameter '" +
      mountPoint + "'");
  assert(typeof path === "string", "Invalid path parameter '" + mountPoint +
      "'");
 ...
 *            mountPoint
 */
ContentDirectoryService.prototype.addHistoryDirectory = function(mountPoint) {
  assert(typeof mountPoint === "string", "Invalid mountPoint parameter '" +
      mountPoint + "'");
 ...
 */
ContentDirectoryService.prototype.addIceCast = function(mountPoint) {
  assert(typeof mountPoint === "string", "Invalid mountPoint parameter '" +
      mountPoint + "'");
...
ContentDirectoryService.prototype._prepareAttributesFilter = function(
  ...
  xmlns[ContentDirectoryService.UPNP_METADATA_XMLNS] = "upnp";
  ...
  filterExpression.split(',').forEach(function(token) {
    ...
    namespaceURIs["dc"] = ContentDirectoryService.PURL_ELEMENT_XMLS;
    namespaceURIs["upnp"] = Service.UPNP_METADATA_XMLNS;
            ...
            function processList(list, node) {
              ...
              var xmlDidl = {
                ...
                _attrs : {
                  ...
                  "xmlns:upnp" : ContentDirectoryService.UPNP_METADATA_XMLNS
                }
              };
              ...
              if (repositoryRequest.dlnaSupport) {
                xmlDidl._attrs["xmlns:dlna"] = ContentDirectoryService.DLNA_METADATA_XMLNS;
              }
  ...
  this
      .getNodeById(
          ...
          function(error, node) {
            ...
            self.getNodeJXML(node, null, repositoryRequest, function(error,
                nodeJXML) {
              if (error) {
