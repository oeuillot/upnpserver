...
ContentDirectoryService.prototype._prepareFilterCallback = function(
  ...
  filterExpression.split(',').forEach(
      function(token) {
        ...
        // as this depends on this server nodes attributes
        // Lame FREEBOX xmlns declaration !!!!
        namespaceURIs[""]     = namespaceURIs[""] || Xmlns.DIDL_LITE;
        namespaceURIs["dc"]   = namespaceURIs["dc"] || Xmlns.PURL_ELEMENT;
        namespaceURIs["upnp"] = namespaceURIs["upnp"] || Xmlns.UPNP_METADATA;
        namespaceURIs["sec"]  = namespaceURIs["sec"] || Xmlns.SEC_DLNA_XMLNS;
...
ContentDirectoryService.prototype.processSoap_Search = function(xml, request, callback) {

  var self = this;

  // use .get() to set default when null or empty string
  // use direct access return null when not found or empty string
  var objectId        = self.soapVars.get("ContainerID",  this.root.id);
  ...
  node = self.soapVars["Filter"];
  if (node) {
    // var fs = node;
    // console.log(Util.inspect(node));
    var filter = Service._childNamed(xml, "Filter", Xmlns.UPNP_SERVICE);
    // NOTE :
    // we cant rely only on Filter node xmlns
    // the namespaces allowed for seek depends on this
    // server nodes xmlns attributes
    filterCallback = this._prepareFilterCallback(node, filter.namespaceURIs);
  }
  ...
  return this.responseSearch(request, objectId, filterCallback,
...
ContentDirectoryService.prototype.responseSearch = function(
  request, containerId, filterCallback, startingIndex, requestedCount, sortCriteria,
...
ContentDirectoryService.prototype.processSoap_Browse = function(xml, request, callback) {
    ...
    var requestedCount  = self.soapVars.get("RequestedCount", -1);
    var sortCriteria    = self.soapVars["SortCriteria"];
    var browseFlag      = self.soapVars["BrowseFlag"];

    var filterCallback = null;
    var node = self.soapVars["Filter"];
    if (node) {
        var filter = Service._childNamed(xml, "Filter", Xmlns.UPNP_SERVICE);
      // var fs = node;
      // console.log(Util.inspect(node));
...
ContentDirectoryService.prototype.processBrowseDirectChildren = function(
  request, containerId, filterCallback, startingIndex,
