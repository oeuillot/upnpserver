...
class ContentDirectoryService extends Service {
  ...
  processNodeContent(node, request, response, path, parameters, callback) {
    ...
    if (contentHandlerKey !== undefined) {
      var contentHandler = this.contentHandlersByName[contentHandlerKey];
      ...
      if (!contentHandler) {
        logger.error('Content handler not found: ' + contentHandlerKey+' for node #'+node.id);

        response.writeHead(404, 'Content handler not found: ' + contentHandlerKey);
        response.end();
        return callback(null, true);
      }
