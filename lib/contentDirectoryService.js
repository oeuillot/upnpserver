...
ContentDirectoryService.prototype.processSoap_Search = function(xml, request, callback) {
  ...
  var node = self.soapArgs["ContainerID"];
  if (node !== null) {
    objectId = node;
  }

  var searchCriteria = null;
  node = self.soapArgs["SearchCriteria"];
  if (node) {
    searchCriteria = node;
  }

  var filterCallback = null;
  node = self.soapArgs["Filter"];
  if (node) {
    // var fs = node;
  ...
  var startingIndex = -1;
  node = self.soapArgs["StartingIndex"];
  if (node !== null) {
    startingIndex = node;
  }

  var requestedCount = -1;
  node = self.soapArgs["RequestedCount"];
  if (node !== null) {
    requestedCount = node;
  }

  var sortCriteria = null;
  node = self.soapArgs["SortCriteria"];
  if (node) {
    sortCriteria = node;
  }
            ...
            function processList(list, node) {
              ...
              Async
                ...
                // and Markup) before embedding in a SOAP response message
                        ...
                        self.soapArgs["Result"] = didl;
                        self.soapArgs["NumberReturned"] = lxml.length;
                        self.soapArgs["TotalMatches"] = total;
                        self.soapArgs["UpdateID"] = updateId;
...
ContentDirectoryService.prototype.processSoap_Browse = function(xml, request, callback) {
    ...
    var objectId = this.root.id;
    var node = self.soapArgs["ObjectID"];
    if (node !== null) {
      objectId = node;
    }
...
ContentDirectoryService.prototype.processBrowseMetadata = function(
  ...
  // and Markup) before embedding in a SOAP response message
              ...
              self.soapArgs["Result"] = didl;
              self.soapArgs["NumberReturned"] = 1;
              self.soapArgs["TotalMatches"] = 1;
              self.soapArgs["UpdateID"] = updateId;
              callback(null);
          }
