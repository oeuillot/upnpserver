...
ContentDirectoryService.prototype.getNodeById = function(id, options, callback) {
  ...
  this._nodeRegistry.getNodeById(id, function(error, node) {
    if (error) {
      return callback(error);
    }
    ...
    self.prepareNodeAttributes(node, callback);
  });
};
...
ContentDirectoryService.prototype.prepareNodeAttributes = function(node,
  ...
  this._preparingQueue.push(node, (callback) ? 0 : 10, function(error) {
...
ContentDirectoryService.prototype._preparingNodeWorker = function(node,
    callback) {
  if (debugWorker.enabled) {
    debugWorker("PrepareNodeWorker node #" + node.id + " " + node.prepared);
  }
  ...
  var contentHandlers = this.listContentHandlers(node);
  if (!contentHandlers || !contentHandlers.length) {
    return callback();
  }

  Async.eachSeries(contentHandlers, function(contentHandler, callback) {
...
ContentDirectoryService.prototype.listContentHandlers = function(node) {
  ...
  var contentHandlersByMimeType = this.contentHandlersByMimeType;
  ...
  if (c2) {
    contentHandlersByMimeType[mime] = c2 || null;
  }
