...
ContentDirectoryService.prototype._prepareFilter = function(filterExpression) {
...
ContentDirectoryService.prototype.processSoap_Browse = function(xml, request,
  ...
  if (node && node.val) {
    ...
    filter = this._prepareFilter(node.val);
  }
          ...
          function(error, item) {
            ...
            function processList(list, node) {
              ...
              Async.eachSeries(list, function(child, callback) {
                if (!child) {
                  logger.warn("ALERT not an node ", child);
                  return callback(null, list);
                }

                child.resolveLink(function(error, childRef) {
                  if (error) {
                    return callback(error);
                  }

                  self.getNodeJXML(childRef, repositoryRequest, function(error,
                      itemJXML) {
                    if (error) {
                      return callback(error);
                    }
                    ...
                    if (childRef !== child) {
                      itemJXML._attrs.id = child.id;
                      itemJXML._attrs.refID = childRef.id;
                      itemJXML._attrs.parentID = child.parentId;
                    }

                    if (filter) {
                      itemJXML = filter(itemJXML);
                    }
                ...
                sortCriteria = sortCriteria || node.upnpClass.defaultSort;
...
ContentDirectoryService.prototype.browseNode = function(item, callback) {
  ...
  if (debug.enabled) {
    debug("CDS: browseNode itemID=" + item.id + " path='" + path +
...
ContentDirectoryService.prototype.newNode = function(parent, name, upnpClass,
  ...
  this.createNode(name, upnpClass, attributes, function(error, node) {
    ...
    parent.insertBefore(node, null, function(error) {
      if (error) {
        logger.error("Append child error=", error);
        return callback(error);
      }
...
ContentDirectoryService.prototype.newContainer = function(parent, name,
  ...
  this.newNode(parent, name, upnpClass || UpnpContainer.UPNP_CLASS, attributes,
      before, callback);
};
...
ContentDirectoryService.prototype.newFile = function(parent, path, upnpClass,
  ...
  if (stats) {
    if (!this.upnpServer.configuration.strict && !attributes.size) {
  ...
  this.newNode(parent, name, upnpClass, attributes, before, callback);
};
...
ContentDirectoryService.prototype.removeNodeById = function(id, callback) {
  this._nodeRegistry.removeNodeById(id, callback);
};
