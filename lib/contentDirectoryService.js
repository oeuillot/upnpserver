...
ContentDirectoryService.prototype._installRoot = function(callback) {
  ...
  var self = this;
  this.initializeRegistry(function(error) {

    self._nodeRegistry.getNodeById(0, function(error, node) {
      if (error) {
        return callback(error);
      }
      if (node) {
...
ContentDirectoryService.prototype.allocateNodesForPath = function(path,
  ...
  var self = this;
  Async.reduce(ps, this.root, function(parentNode, segment, callback) {

    parentNode.getChildByName(segment, function(error, node) {
      if (error) {
        return callback(error);
      }
      ...
      self.newNode(parentNode, segment, UpnpContainer.UPNP_CLASS, {
        virtual : true
      }, callback);
    });
  }, callback);
  ...
  this
      .getNodeById(
          ...
          function(error, node) {
            ...
            self.getNodeJXML(node, null, repositoryRequest,
                filterSearchEngine.func, function(error, nodeJXML) {
                  if (error) {
...
ContentDirectoryService.prototype.createNode = function(name, upnpClass,
  ...
  Node.create(this, name, upnpClass, attributes, function(error, node) {
    if (error) {
      return callback(error);
    }
    ...
    upnpClass.prepareNode(node, function(error) {
      ...
      self.registerNode(node, callback);
    });
  });
};
...
ContentDirectoryService.prototype.newNode = function(parentNode, name,
  ...
  this.createNode(name, upnpClass, attributes, function(error, node) {
    if (error) {
      console.error("Can not create node #", node.id, "error=", error);
      return callback(error);
    }

    parentNode.insertBefore(node, before, function(error) {
      if (error) {
        console.error("Append child error #", node.id, "error=", error);
        return callback(error);
      }
...
ContentDirectoryService.prototype.registerNode = function(node, callback) {
  ...
  this._nodeRegistry.registerNode(node, function(error) {
    ...
    self.asyncEmit('newNode', node, function() {
      callback(null, node);
    });
  });
};
...
ContentDirectoryService.prototype.saveNode = function(node, modifiedProperties,
    callback) {
  var self = this;
  this._nodeRegistry.saveNode(node, modifiedProperties, function(error) {
    if (error) {
      return callback(error);
    }

    self.asyncEmit('saveNode', node, modifiedProperties, callback);
  });
};
...
ContentDirectoryService.prototype.unregisterNode = function(node, callback) {
  var self = this;
  this.asyncEmit('deleteNode', node, function(error) {
    if (error) {
      return callback(error);
    }
    self._nodeRegistry.unregisterNode(node, callback);
  });
};
...
ContentDirectoryService.prototype._preparingNodeWorker = function(node,
  ...
  node._take("preparing", function() {
    ...
    self.emitPrepareNode(node, function(error) {
      debugWorker("PrepareNodeWorker node #", node.id, " done error=", error);

      self.saveNode(node, null, function(error, newNode) {
        node._leave("preparing");
