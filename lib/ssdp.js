...
SsdpServer.prototype.addRoot  = function( uuid, usn, location, callback ){
  var root = {
    location: location,
    usn:{},
    processVendorHeaders:callback
  };
  // [1] page 20 table 1-1
...
SsdpServer.prototype.addDevice = function( uuid, usn, location, callback){
    var device = {
      location: location,
      usn:{},
      processVendorHeaders:callback
...
SsdpServer.prototype.advertise = function (alive) {
  ...
  Object.keys(self._devices).forEach(function (uuid) {
      ...
      Object.keys(device.usn).forEach(function(usn){
        ...
        device.processVendorHeaders(heads, alive);
        ...
        self.ips.forEach(function(host){
          ...
          self.clients[host].send(message, 0, message.length, self._ssdpPort, self._ssdpIp, function (err, bytes) {
            self._logger.trace({'message': message.toString()}, 'Advertise ', heads.NTS)
          });
  ...
  Object.keys(self._devices).forEach(function (uuid) {
      ...
      Object.keys(device.usn).forEach(function(usn){
        ...
        if (serviceType === 'ssdp:all' || usn === serviceType) {
          ...
          device.processVendorHeaders(heads, true);
          ...
          self._send(message, peer, port, function (err, bytes) {
            self._logger.trace({'peer': peer, 'port': port, 'message': pkt}, 'Sent M-SEARCH response')
          })
        }
      })
  })
