...
class Node {
  ...
  static createRef(linkedNode, name, callback) {
    ...
    if (ASSERT_CAN_NOT_LINK_A_CONTAINER && linkedNode.isContainer) {
      ...
      service.saveNode(this, {
        ...
        service.unregisterNode(child, (error) => {
          ...
          service.getNodeById(refId, (error, refNode) => {
            ...
            refNode.removeLink(child, callback);
          });
        });
      });
    });
  ...
  getFirstVirtualChildByTitle(title, callback) {
    ...
    this.listChildrenByTitle(title, (error, nodes) => {
      ...
      for(var node of nodes) {
        ...
        debugChildByTitle("getFirstVirtualChildByTitle: Find a virtual title=",title," in #",this._id, "=> #", node._id);
        return callback(null, node);
      }

      debugChildByTitle("getFirstVirtualChildByTitle: Can not find virtual title=",title," in #",this._id);
  ...
  resolveLink(callback) {
    ...
    this._service.getNodeById(this.refId, (error, child) => {
      ...
      logger.error("Can not find refId #"+this.refId+" of node #"+this.id);
