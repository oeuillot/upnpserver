...
/**
 * filterChildNodes : recursievely filter all childs of a node
 *
 * @param {Node}       parent
 * @param {array|null} list
 * @param {function}   filter   : argument node, return true or false
 * @param {function}   callback : argument list, called on complete
 */
  ...
  this.listChildren(function(error, children) {
    ...
    Async.eachSeries(links, function(link, callback) {
      ...
      link.resolveLink(function(error, node) {
        ...
        if (node.name === name) {
          // callback with an error set to break the series
