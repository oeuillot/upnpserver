...
class Node {
  ...
  listChildrenByTitle(title, callback) {
    this._listChildrenByTitle(title, (error, nodesIds, nodes) => {
      ...
      if (nodes) {
        debug("listChildrenByTitle: DIRECT return nodes.count=",nodes.length);
        return callback(null, nodes);
      }
      if (!nodesIds) {
        debug("listChildrenByTitle: DIRECT return EMPTY");
        return callback(null, []);
      }
      ...
      Async.mapLimit(nodesIds, LIST_CHILDREN_LIMIT, (id, callback) => service.getNodeById(id, callback), 
          (error, nodes) => {
            ...
            debug("listChildrenByTitle: Computed return");
  ...
  _listChildrenByTitle(title, callback) {
    ...
    this.takeLock("childrenByTitle", () => {
      ...
      this.eachChild((node, link) => {
        ...
        var linkTitle = (link.attributes && link.attributes.title); // Not the name || link.name;
