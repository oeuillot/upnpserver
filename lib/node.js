...
const CAN_NOT_LINK_A_CONTAINER = false;
...
class Node {
  ...
  static createRef(linkedNode, name, callback) {
    ...
    if (CAN_NOT_LINK_A_CONTAINER && linkedNode.isContainer) {
  ...
  browseChildren(options, callback) {
    ...
    this.service.browseNode(this, (error) => {
      if (error) {
  ...
  getFirstVirtualChildByTitle(title, callback) {
    debug("getFirstVirtualChildByTitle: request for ",title);
    ...
    this.listChildrenByTitle(title, (error, nodes) => {
      ...
      for(var i=0;i<nodes.length;i++) {
        ...
        if (!node.contentURL) {
          debug("getFirstVirtualChildByTitle: Find a virtual title=",title," in #",this._id, "=> #", node._id);
          return callback(null, node);
        }
      }

      debug("getFirstVirtualChildByTitle: Can not find virtual title=",title," in #",this._id);
  ...
  listChildrenByTitle(title, callback) {
    debug("listChildrenByTitle: request for ",title);
    this._listChildrenByTitle(title, (error, nodesIds, nodes) => {
      if (error) {
        return callback(error);
      }
      if (nodes) {
        debug("listChildrenByTitle: DIRECT search of", title, "return nodes.count=",nodes.length);
        return callback(null, nodes);
      }
      if (!nodesIds) {
        debug("listChildrenByTitle: DIRECT search of", title, "return EMPTY");
        return callback(null, []);
      }

      var service=this.service;

      Async.mapLimit(nodesIds, LIST_CHILDREN_LIMIT, (id, callback2) => service.getNodeById(id, callback2), 
          (error, nodes) => {
            if (error) {
              return callback(error);
            }

            debug("listChildrenByTitle: Computed search of", title, "return count=", nodes.length);
  ...
  _listChildrenByTitle(title, callback) {
    debug("_listChildrenByTitle: request for ",title);
    ...
    if (childrenByTitle) {
      ...
      debug("_listChildrenByTitle: HIT! #",this.id,"title=",title,"=>",children);
    ...
    this.takeLock("childrenByTitle", () => {
      this._mapChildrenByTitle(title, (error, childrenByTitle, nodes) => {
        ...
        debug("_listChildrenByTitle: SyncedHIT! #",this.id,"title=",title,"=>",children);
  ...
  mapChildrenByTitle(callback) {
    debug("mapChildrenByTitle: request map ...");
    
    var map=this.service._childrenByTitleWeakHashmap;

    var childrenByTitle=map.get(this._id, this);
    if (childrenByTitle) {
      debug("mapChildrenByTitle: map in cache");
      return callback(null, childrenByTitle);
    }

    this.takeLock("childrenByTitle", () => {
      this._mapChildrenByTitle(null, (error, childrenByTitle) => {
        this.leaveLock("childrenByTitle");
   ...
   * 
   */
  _mapChildrenByTitle(title, callback) {
    debug("_mapChildrenByTitle: request map title=",title);

    var map=this.service._childrenByTitleWeakHashmap;

    var childrenByTitle=map.get(this._id, this);
    if (childrenByTitle) {
      debug("_mapChildrenByTitle: map in cache !");
      return callback(null, childrenByTitle);
    }
    ...
    this.eachChild((node, link) => {
      ...
      var linkTitle = (link.attributes && link.attributes.title) || link.name;
      ...
      debug("_mapChildrenByTitle: map=",childrenByTitle);
