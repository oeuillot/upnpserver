...
class Node {
  ...
  removeChild(child, callback) {
    ...
    if (idx < 0) {
      let ex = new Error("Can not find child #" + child.id);
      ex.node = this;
      ex.child = child;
      return callback(ex);
    }
    ...
    if (child.childrenIds && child.childrenIds.length) {
      let ex = new Error("Can not remove child #" + child.id +
    ...
    this._take("children", () => {
      ...
      service.saveNode(this, {
        ...
        if (error) {
          console.error("Can not save node #", this.id, error);
          this._leave("children");
          return callback(error);
        }
  ...
  insertBefore(child, before, callback) {
    ...
    this._take("children", () => {
      ...
      if (typeof (before) === "number") {
        ...
        if (idx < 0) {
          let ex = new Error("Before child #" + before.id + " is not found");
          ex.node = this;
      ...
      if (!this._path) {
        ...
        console.error("**** Not connected to the root ? #" + this.id, "name=",
  ...
  listChildren(options, callback) {
    ...
    this._take("children", () => {
      ...
      if (canUseCache) {
        var cache = service._childrenWeakHashmap.get(this.id, this);
        if (cache) {
      ...
      if (childrenIds !== undefined) {
        if (LOG_LIST_CHILDREN) {
          debug("Node.listChildren #", this.id, "=> cached ids list ",
        ...
        Async.mapLimit(childrenIds, LIST_CHILDREN_LIMIT, (id, callback) => {
          ...
          if (error) {
            console.error(error, error.stack);
            if (debug.enabled) {
              debug("Node.listChildren[#" + this.id+ "] => map returs error ",
                  error);
            }
          ...
          if (LOG_LIST_CHILDREN) {
            logger.debug("Node.listChildren[#" + this.id + "] => map returs " +
                result);
          }
      ...
      if (LOG_LIST_CHILDREN) {
        debug("Node.listChildren #" + this.id + "=> not in cache !");
      }
      ...
      service.browseNode(this, (error, list) => {
        ...
        if (LOG_LIST_CHILDREN) {
          debug("Node.listChildren #", this.id, "=>", list);
        }
  ...
  getChildByName(name, callback) {
    this.findChild((node) => {
      if (LOG_GET_CHILD_BYNAME) {
        debug("Node.getChildByName #", this.id, "name=", name, "=>", node.id);
      }
  ...
  findChild(testFunc, callback) {
    this.listChildren((error, children) => {
      ...
      if (!links) {
        if (LOG_GET_CHILD_BYNAME) {
          logger.debug("Node.getChildByName[#" + this.id+ "] => NO RESULT");
        }
  ...
  _treeString(indent, callback) {
    ...
    console.log("TreeString #",this.id);
  ...
  getAttributes(options, callback) {
    ...
    debug("Requesting prepare node #",this.id," options=",options);

    this._service.prepareNodeAttributes(this, options, (error, node) => {

      debug("Requesting prepare node #",this.id," returns error=",error);
  ...
  _take(lockName, callback) {
    ...
    if (!semaphore) {
      semaphore = new Semaphore("Node#"+this.id+":"+lockName);
