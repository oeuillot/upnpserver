...
var StateVar = module.exports = function(service, name, type, errCode, val,
  ...
  if (evented === 2){
    ...
    var throttle = self.throttle(self.notify, self.rate);
    self.set = function(val) {
      var old = self.value;
      self.value = val;
      if (old !== val) {
 ...
 *
 */
StateVar.prototype.throttle = function(func, limit) {
    ...
    return function () {
        ...
        if (!wait) {
            ...
            // disallow immediate call right after waiting period
            if (immediate){
            ...
            // time left between two calls to keep rate
            // when no more waiting and immediate disabled
            var delay = limit + lastCall - Date.now();

            setTimeout(function () {
                // call on waiting period end
                func.apply(context, args);
                lastCall = Date.now();
                wait = false;
...
StateVar.prototype.debounce = function(func, wait, immediate) {
