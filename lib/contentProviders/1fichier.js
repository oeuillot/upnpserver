...
class OneFichierContentProvider extends ContentProvider {
  ...
  readdir(contentURL, callback) {
    ...
    this._readdir(url, folderId, callback);
  }
  ...
  stat(contentURL, callback) {
    ...
    if (true) {
      ...
      url=this._baseURL+"console/get_folder_content.pl?id="+reg[1];
    ...
    url=this._convertURL(contentURL);
