...
const debug = require('debug')('upnpserver:repository:Directory');
...
class DirectoryRepository extends PathRepository {
  ...
  addDirectory(parentNode, contentURL, stats, callback) {
    ...
    parentNode.listChildrenByTitle(name, (error, nodes, links) => {
      if (error) {
        logger.error("addDirectory name="+name, error);
        return callback(error);
      }
      ...
      for(var i=0;i<nodes.length;i++) {
        ...
        debug("Compare directory",node.contentURL,"<>",contentURL,stats.isDirectory(),"<>",node.isContainer);
  ...
  addFile(parentNode, contentURL, stats, callback) {
    ...
    parentNode.listChildrenByTitle(name, (error, nodes, links) => {
      if (error) {
        logger.error("listChildrenByTitle name="+name, error);
        return callback(error);
      }
      ...
      for(var i=0;i<nodes.length;i++) {
        ...
        debug("Compare file",node.contentURL,"<>",contentURL,node.contentTime,"<>",stats.mtime.getTime(),
            stats.isDirectory(),"<>",node.isContainer);
