...
class DirectoryRepository extends PathRepository {
  ...
  constructor(repositoryId, mountPath, path, searchClasses) {
    ...
    super(repositoryId, mountPath, path, searchClasses);
  }
    ...
    contentProvider.readdir(path, (error, files) => {
      if (error) {
        if (error.code === "ENOENT") {
          ...
          debug("DirectoryRepository: ENOENT for " + path);
      ...
      Async.eachLimit(files, BROWSE_FILES_LIMIT, (file, callback) => {
        ...
        contentProvider.stat(p, (error, stats) => {
          debug("DirectoryRepository: child=",p,"stats=",stats);
          if (error) {
          ...
          if (stats.isDirectory()) {
            this.addDirectory(node, p, stats, (error, node) => {
              if (error) {
          ...
          if (stats.isFile()) {
            this.addFile(node, p, stats, (error, node) => {
        ...
        debug("DirectoryRepository: END browse=", itemPath, " path=", path,
   ...
   * 
   */
  addDirectory(parentNode, contentURL, stats, callback) {
    var name = contentURL;
    var ret = /\/([^/]+$)/.exec(name); // We can not use basename, because of Win32
    name = (ret && ret[1]) || name;

    parentNode.getFirstChildByName(name, (error, node) => {
      if (error) {
        logger.error("GetFirstChildByName name="+name, error);
        return callback(error);
      }

      if (node && node.contentURL === contentURL && stats.isDirectory() && node.isContainer) {
      ...
      this.newFolder(parentNode, contentURL, null, stats, null, callback);
    });
  }
  ...
  addFile(parentNode, contentURL, stats, callback) {
    ...
    parentNode.getFirstChildByName(name, (error, node) => {
      ...
      if (node && node.contentURL === contentURL &&
          node.contentTime === stats.mtime && stats.isFile() && !node.isContainer) {
