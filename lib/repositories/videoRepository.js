...
VideoRepository.prototype.registerMovie = function(parentNode, title, itemData,
  ...
  if (!exploded) {
    parentNode.listChildren(function(error, list) {
      ...
      // if (list.length < 100) {
      return self.registerMovie2(parentNode, title, itemData, callback);
      // }
...
VideoRepository.prototype.registerMovie2 = function(parentNode, title,
  ...
  var self = this;
  parentNode.getChildByName(title, function(error, movieItem) {
    if (error) {
      return callback(error);
    }
    ...
    if (itemData.movieItem) {
      ...
      return self.newNodeRef(parentNode, itemData.movieItem, null, function(
          error, item) {
        if (error) {
          return callback(error);
        }
