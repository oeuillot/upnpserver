...
class IceCastRepository extends Repository {
  ...
  initialize(service, callback) {
    ...
    var scan = (root, nodes, callback) => {
      ...
      this.scan(root, nodes, (error, root, list, chidrens) => {
        if (error) {
          return callback(error);
        }

        this.update(root, list, chidrens, (error) => {
          if (error) {
            logger.error(error);
          }
    ...
    super.initialize(service, (error, root) => {
      ...
      setInterval(() => {
        ...
        // clean up childs
        debug("start update");
        this.getNodeList(root, (err, list) => scan(root, list));
  ...
  getNodeList(parent, callback) {
    ...
    parent.filterChildNodes((parent) => {
  ...
  processFile(rootItem, attributes, callback) {
    ...
    this.service.createNode(name, AudioBroadcast.UPNP_CLASS, attributes, false, null, (error, node) => {
      if (error) {
        return callback(error);
      }
  ...
  registerAudioBroadcast(parentItem, itemData, tryCount, callback) {
    ...
    parentItem.getFirstVirtualChildByTitle(t, (error, audioBroadcast) => {
      if (error) {
        return callback(error);
      }
      ...
      if (itemData.audioBroadcast) {
        ...
        return this.newNodeRef(parentItem, itemData.audioBroadcast, callback);
      }
  ...
  registerGenresFolder(parentItem, itemData,   genreName, callback) {
    ...
    parentItem.getFirstVirtualChildByTitle(genreName, (error, genreItem) => {
      if (error) {
        return callback(error);
      }
      ...
      this.newVirtualContainer(parentItem, genreName, MusicGenre.UPNP_CLASS, (error, genreItem) => {
