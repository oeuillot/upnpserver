...
var debug = require('debug')('device:repository');
...
var Repository = function(configuration) {

  var repositoryId;

  if (configuration.path){
    repositoryId = configuration.type + ":" + configuration.path;
  }

  this.repositoryId = repositoryId || Uuid.v4();

  var mountPoint    = configuration.mountPoint || "";
...
Repository.prototype.initialize = function(service, callback) {
  ...
  var self = this;
  service.allocateItemsForPath(this.mountPoint, function(error, node) {
    if (error) {
...
Repository.prototype._fillAttributes = function(attributes, stats) {
  if (!this.service.device.configuration.strict &&
      !attributes.size) {
...
Repository.prototype.newFile = function(parentNode, contentURL, upnpClass,
  ...
  function processStats(stats) {
    ...
    if (!upnpClass && attributes.mime) {
      var byMimeType = self.service.upnpClassesByMimeType;
    ...
    return self.service.newNode(parentNode, name, upnpClass,
  ...
  this.service.getContentProvider(contentURL).stat(contentURL,
...
Repository.prototype.newVirtualContainer = function(parentNode, name,
  ...
  this.service.newNode(parentNode, name, upnpClass, attributes,
      before, callback);
};
...
Repository.prototype.newNodeRef = function(parentNode, targetNode, name,
  ...
  this.service.newNodeRef(parentNode, targetNode, name, before,
      callback);
};

Repository.prototype.keepFile = function(infos) {
  return false;
};
/**
 * Add file(s) on startup or when filesystem (or whatever) change
 */
Repository.prototype.processFile = function(node, contentURL, infos, callback) {
