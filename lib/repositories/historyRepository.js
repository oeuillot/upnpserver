...
const debug = require('debug')('upnpserver:repository:History');
...
class HistoryRepository extends Repository {
  ...
  _declareFolders(parentNode, clientId,
      callback) {
  ...
  _registerNewRef(nodeType, node,
    ...
    this._declareFolders(this._mountNode, clientId, (error, folderNode) => {
      ...
      this._removeNodeRef(parent, node, this.perHostHistorySize, (error) => {
        ...
        this.newNodeRef(parent, node, (error, newNode) => {
          if (error) {
            return callback(error);
          }
          ...
          this._declareFolders(this._allNode, "*", (error, folderNode) => {
            ...
            this._removeNodeRef(parent, node, this.allHostHistorySize, (
                error) => {
                  if (error) {
                    return callback(error);
                  }
