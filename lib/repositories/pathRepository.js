  ...
  contentProvider.readdir(path, function(error, files) {
    ...
    Async.eachLimit(files, BROWSE_FILES_LIMIT, function(file, callback) {
      ...
      contentProvider.stat(p, function(error, stats) {
        ...
        if (stats.isFile()) {
          return self.addFile(node, p, stats, function(error, node) {
...
PathRepository.prototype.addDirectory = function(parentNode, contentURL, stats,
  ...
  parentNode.getChildByName(name, function(error, node) {
    if (error) {
      return callback(error);
    }

    if (node && node.attributes.contentURL === contentURL &&
        stats.isDirectory()) {

      console.log("Directory ALREADY EXISTS " + node.id);
...
PathRepository.prototype.addFile = function(parentNode, contentURL, stats,
    callback) {
  var name = contentURL;
  var ret = /\/([^/]+$)/.exec(name); // We can not use basename, because of Win32
  name = (ret && ret[1]) || name;
  ...
  parentNode.getChildByName(name, function(error, node) {
    ...
    if (node && node.attributes.contentURL === contentURL && stats.isFile() &&
        node.attributes.modifiedTime === stats.mtime) {

      console.log("File ALREADY EXISTS " + node.id);

      return callback(null, node);
    }

    return this.newFile(parentNode, contentURL, null, stats, null, callback);
  });
};
