...
class IceCastRepository extends Repository {
  ...
  processFile(rootItem, attributes, callback) {
    ...
    this.contentDirectoryService.createNode(name, AudioBroadcast.UPNP_CLASS, attributes, false, null, (error, node) => {
      if (error) {
        return callback(error);
      }
  ...
  registerAudioBroadcast(parentItem, itemData, tryCount, callback) {
    ...
    parentItem.getFirstChildByName(t, (error, audioBroadcast) => {
      if (error) {
        return callback(error);
      }
  ...
  registerGenresFolder(parentItem, itemData,   genreName, callback) {
    ...
    parentItem.getFirstChildByName(genreName, (error, genreItem) => {
      if (error) {
        return callback(error);
      }
