...
IceCastRepository.prototype._parsePlaylist  = function (body, callback){
    ...
    if ((/DOCTYPE HTML/g).test(body) || !body){
        // skip parsing when server respond with 403 Forbidden
        return callback("Error in IceCastRepository parsePlaylist : body is not a valid json content (DOCTYPE HTML)");
    }
    ...
    try{
      ...
      return callback("Error in IceCastRepository parsePlaylist : body is not a valid json content");
    }
      ...
      fs.stat(ICECAST_CACHE, function(err, stats){
          ...
          if (!self.dirty && !err && stats.isFile() && (stats.mtime.getTime() + ICECAST_UPDATEDELAY <  now)){
              ...
              fs.readFile(ICECAST_CACHE, function(err, body){
                  if (err) return callback(err);
                  var files = self._parsePlaylist(body, function(err, files){
              ...
              var req = http.request(ICECAST_API, function(res){
                  ...
                  res.on('end', function() {
                      debug("save cache file");
                        self._parsePlaylist(body, function(err, files){
                          if (!err){
                          // only save cache for successfully parsed files
