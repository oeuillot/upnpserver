...
NodeWeakHashmap.prototype.get = function(id, node) {
  if (!this._count) {
    return undefined;
  }
  ...
  v.date = (this._intervalId) ? this._now : (Date.now() + this._delay);
  return v.value;
};
...
NodeWeakHashmap.prototype.clear = function() {
  this._map = {};
  this._count = 0;
};
...
NodeWeakHashmap.prototype._garbage = function() {
  ...
  if (gs) {
    gs.forEach(function(v) {
      try {
        garbageFunc(v.value, k, v.date, v.updateId);
