...
var RootDevice = function( api, _configuration, callback) {
  ...
  this.descriptionEndpoint = this.configuration.descriptionEndpoint || "description.xml"
  ...
  this.startHttpServer(function(){
    ...
    Async.forEachOfSeries(self.subDevices, function(sub, key, callback){
      ...
      var location = ':' + self.port + subroute + '/' + self.descriptionEndpoint;

      self.ssdp[addmethod](sub.uuid, sub.type, location, sub.ssdpHeadersCallback.bind(sub));
...
RootDevice.prototype.processRequest = function(request, response, path,
  ...
  switch (segment){
    ...
    case self.descriptionEndpoint:{
      return this.toJXML(request, response, path, callback);
    }
    break;
    default:{
 ...
 *            callback
 */
RootDevice.prototype.toJXML = function(request, response, path,
				       callback) {
    var self = this;
    var now;
    
    
    return this.rootDevice.toJXML("/" + this.rootDevice.route, 
				  request, 
    	         function(error, xmlObject) {
		     if (error) {
			 return callback(error);
		     }
