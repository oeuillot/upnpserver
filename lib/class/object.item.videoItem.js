...
class VideoItem extends Res {
  ...
  processRequest(node, request, response, path,
    ...
    return super.processRequest(node, request, response, path,
        parameters, callback);
  }

  processSrtRequest(node, request, response, path,
      parameters, callback) {
    ...
    node.service.getContentProvider(srtPath).stat(
        ...
        (error, stats) => {
          ...
          if (error) {
            console.error(error);
            return callback("No srt ? (" + srtPath + ")" + error,
                true);
          }
