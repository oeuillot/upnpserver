...
class Res extends Item { 
  ...
  toJXML(node, attributes, request, filterCallback,
    ...
    super.toJXML(this, node, attributes, request, filterCallback,
        (error, xml) => {
          if (error) {
          ...
          if (filterCallback(Xmlns.DIDL_LITE, "res")) {
            ...
            if (!res &&
                (node.contentURL || attributes.externalContentURL)) {
  ...
  processRequest(node, request, response, path,
    ...
    super.processRequest(node, request, response, path, parameters,
        (error, processed) => {
          if (error || processed) {
            return callback(error, processed);
          }

          var realpath = node.contentURL;
...
function _addRes(xml, res, request, node, filterCallback) {
  ...
  if (filterCallback(Xmlns.DIDL_LITE, "res", "protocolInfo")) {
    ...
    if (!contentFormat && !res.key) {
      contentFormat = attributes.mime || Mime.lookup(node.contentURL || '');
    }
