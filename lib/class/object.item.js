...
const _UPNP_CLASS = ObjectClass.UPNP_CLASS + ".item";
...
class Item extends ObjectClass {
  ...
  toJXML(node, attributes, request, filterCallback, callback) {
    ...
    assert(node.id!==undefined, "Invalid node.id");
    assert(node.parentId!==undefined, "Invalid node.parentId");
    ...
    if (attributes.year && request.jasminFileMetadatasSupport) {
      if (filterCallback(Xmlns.JASMIN_FILEMETADATA, "year")) {
          content.push({
            _name : "fm:year",
            _content : attributes.year
          });
      }
    }
   ...
   * 
   */
  static getXmlNode(node, name, create) {
  ...
  static toISODate(date) {
    ...
    return date.toISOString().replace(/\..+/, ''); // Remove ms
  }
...
function _addFileTimes(node, request, xml, attributes, filterCallback) {
  ...
  if (request.secDlnaSupport) {
    ...
    if (filterCallback(Xmlns.SEC_DLNA, "dcmInfo")) {
      var dcmInfo = Item.getXmlNode(xml, "sec:dcmInfo", false);
      var infos = {};
      if (dcmInfo && dcmInfo._content) {
      ...
      if (ci.length) {
        if (!dcmInfo) {
          dcmInfo = Item.getXmlNode(xml, "sec:dcmInfo", true);
        }
