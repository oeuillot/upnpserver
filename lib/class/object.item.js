...
var FILEMETADATA_XMLNS = "urn:schemas-jasmin-upnp.net:filemetadata/";
...
Item.prototype.toJXML = function(node, attributes, request, callback) {
  ...
  if (request.contentDirectoryServer.jasminFileMetadatasExtension) {
    _addFileTimes(xml, attributes);
  }
  ...
  // console.log("Format ", attributes, "=>", xml);
...
function _addFileTimes(xml, attributes) {
  ...
  FILEMEDATA_TIMES_LIST.forEach(function(time) {
    ...
    Item.addNamespaceURI(xml, "fm", FILEMETADATA_XMLNS);
