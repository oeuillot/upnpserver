...
function _addFileTimes(node, request, xml, attributes, filterCallback) {
  ...
  if (request.secDlnaSupport) {
    ...
    if (filterCallback(Xmlns.SEC_DLNA_XMLNS, "modificationDate")) {
      var modificationDate = attributes["modifiedTime"];
      if (modificationDate) {
        content.push({
    ...
    if (filterCallback(Xmlns.SEC_DLNA_XMLNS, "dcmInfo")) {
      var birthTime = attributes["birthTime"];
      if (birthTime) {
        var dcmInfo = Item._getNode(xml._content, "sec:dcmInfo", true);

        var infos = (dcmInfo._content || "").split(',');
        infos.push("CREATIONDATE=" + Item.toMsDate(birthTime));

        dcmInfo._content = infos.join(',');
      }
    }
  }
