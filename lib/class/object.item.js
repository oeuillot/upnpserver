...
Item.prototype.toJXML = function(node, attributes, request, attributesFilter,
    callback) {
  ...
  if (attributesFilter(Xmlns.PURL_ELEMENT, "title")) {
  ...
  if (attributesFilter(Xmlns.PURL_ELEMENT, "date")) {
    if (attributes.year) {
      content.push({
        _name : "dc:date",
  ...
  if (request.contentDirectoryService.jasminFileMetadatasExtension) {
    _addFileTimes(xml, attributes, attributesFilter);
  }

  if (attributesFilter(Xmlns.UPNP_METADATA, "artist")) {
    Item.addList(content, attributes.artists, "upnp:artist", true);
  }

  if (attributesFilter(Xmlns.UPNP_METADATA, "actor")) {
    Item.addList(content, attributes.actors, "upnp:actor", true);
  }

  if (attributesFilter(Xmlns.UPNP_METADATA, "author")) {
    Item.addList(content, attributes.authors, "upnp:author", true);
  }

  if (attributesFilter(Xmlns.UPNP_METADATA, "producer")) {
    Item.addList(content, attributes.producers, "upnp:producer", false);
  }

  if (attributesFilter(Xmlns.UPNP_METADATA, "director")) {
    Item.addList(content, attributes.directors, "upnp:director", false);
  }

  if (attributesFilter(Xmlns.PURL_ELEMENT, "publisher")) {
    Item.addList(content, attributes.publishers, "dc:publisher", false);
  }

  if (attributesFilter(Xmlns.PURL_ELEMENT, "contributor")) {
    Item.addList(content, attributes.contributors, "dc:contributor", false);
  }

  if (attributesFilter(Xmlns.UPNP_METADATA, "rating")) {
    _addRatings(content, attributes.ratings);
  }

  if (attributesFilter(Xmlns.UPNP_METADATA, "genre")) {
...
function _addFileTimes(xml, attributes, attributesFilter) {
  ...
  FILEMEDATA_TIMES_LIST.forEach(function(time) {
    ...
    if (!attributesFilter(Xmlns.JASMIN_FILEMETADATA, time)) {
      return;
    }
