    ...
    super.toJXML(node, attributes, request,
        filterCallback, (error, xml) => {
          ...
          if (request.jasminMovieMetadatasSupport) {
            ...
            if (filterCallback(Xmlns.JASMIN_MOVIEMETADATA, "season")) {
              if (attributes.season!==undefined) {
                content.push({
                  _name : "mo:season",
                  _content : attributes.season
                });
              }
            }

            if (filterCallback(Xmlns.JASMIN_MOVIEMETADATA, "episode")) {
              if (attributes.episode!==undefined) {
                content.push({
                  _name : "mo:episode",
                  _content : attributes.episode
                });
              }
            }

            if (filterCallback(Xmlns.JASMIN_MOVIEMETADATA, "airDate")) {
              if (attributes.airDate!==undefined) {
                content.push({
                  _name : "mo:airDate",
                  _content : attributes.airDate
                });
              }
            }
          }
