                    ...
                    function(error, list) {
                      ...
                      list
                          .forEach(function(child) {
                            ...
                            if (attributesFilter(Xmlns.UPNP_METADATA,
                                "albumArtURI")) {
                              if (!MAX_ALBUM_ARTS || count < MAX_ALBUM_ARTS) {
                                var albumArts = childAttributes.albumArts;
                                if (albumArts) {
                                  albumArts
                                      .forEach(function(albumArtInfo) {
                                        if (MAX_ALBUM_ARTS > 0 &&
                                            count > MAX_ALBUM_ARTS) {
                                          return;
                                        }

                                        if (ImageItem
                                            .isMimeTypeImage(albumArtInfo.mimeType)) {
                                          var aau = {
                                            _name : "upnp:albumArtURI",
                                            _content : request.contentURL +
                                                child.id + "?contentHandler=" +
                      ...
                      if (attributesFilter(Xmlns.UPNP_METADATA, "artist")) {
                        Item.addList(xml._content, artistsList.length,
                            "upnp:artist", true);
                      }
