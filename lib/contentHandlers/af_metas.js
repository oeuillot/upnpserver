...
class AF_Metas extends Abstract_Metas {
  ...
  getTrailerPath(contentURL, key, callback) {
    callback(null, this.contentProvider.join(this.basePath, key, TRAILER));
  }
  ...
  getPosterPath(contentURL, key, callback) {
    callback(null, this.contentProvider.join(this.basePath, key, POSTER));
  }
  ...
  prepareMetas(contentInfos, context, callback) {
    ...
    var path = this.contentProvider.join(this.basePath, afKey);
  ...
  loadJSON(attributes, afKey, path, callback) {
    var jsonPath = this.contentProvider.join(path, AC_JSON);

    debug("loadJSON", "Load json", jsonPath);
