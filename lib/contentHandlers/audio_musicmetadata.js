...
Audio_MusicMetadata.prototype.prepareNode = function(node, callback) {
  ...
  mm(stream, function(error, tags) {
    ...
    if (error) {
      logger.error("MM can not parse tags of path=", path, " error=", error);
      return callback();
    }
...
Audio_MusicMetadata.prototype.processRequest = function(node, request,
  ...
  this._getPicture(node.attributes.realPath, albumArtKey, function(error,
      picture) {
...
Audio_MusicMetadata.prototype._getPicture = function(path, pictureIndex,
    callback) {
