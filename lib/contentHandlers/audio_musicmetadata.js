...
const debug = require('debug')('upnpserver:contentHandler:audio_musicmetadata');
...
class Audio_MusicMetadata extends ContentHandler {
  ...
  _getPicture(node, contentURL, pictureIndex, callback) {
    ...
    this.service.getContentProvider(node).createReadStream(null, contentURL, null, (error, stream) => {
      if (error) {
        return callback(error);
      }
