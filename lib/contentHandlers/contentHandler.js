...
class ContentHandler {
  ...
  initialize(contentDirectoryService, callback) {
    ...
    var prepareNode = (contentURL, attributes, callback) => {
      debug("[", this.name, "] PrepareNode event of content",contentURL);

      this.prepareMetasFromContentURL(contentURL, attributes, (error) => {
        if (error) {
          logger.error("Prepare node "+contentURL+" of contentHandler="+this.name+" error=",error);
          return callback(error);
        }

        debug("[",this.name,"] PrepareNode event END of content",contentURL);
        callback();
      });
    };
  ...
  prepareMetasFromContentURL(contentURL, attributes, callback) {
    ...
    this.prepareMetas(contentURL, attributes, (error, metas) => {
      if (error) {
        logger.error("loadMetas error", contentURL, contentURL, error);
