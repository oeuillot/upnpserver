...
var ScannerRepository = function(mountPath, path) {
	...
	fs.watch(path, function(event, filename) {
		console.log('ScannerRepository: event is: ' + event);
		if (filename) {
...
ScannerRepository.prototype.initialize = function(service, callback) {
	...
	Repository.prototype.initialize.call(this, service, function(error, item) {
		...
		process.nextTick(function() {
			self.scan(service, function(error) {
				if (error) {
					console.error("ScannerRepository: Scan error", error);
				}
			});
		});
...
ScannerRepository.prototype.scan = function(service, callback) {
	var self = this;
		...
		Async.eachLimit(files, FILES_PROCESSOR_LIMIT,
				function(infos, callback) {
					...
					self.processFile(item, infos, function(error) {
						if (error) {
							console.error("Process file itemId=" + item.itemId
					...
					if (error) {
						console.error("Error while scaning files ", error);
						return callback(error);
					}
