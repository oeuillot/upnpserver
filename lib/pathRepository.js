...
var PathRepository = function(mountPath, path, searchClasses) {
	...
	fs.watch(path, function(event, filename) {
		console.log('PathRepository: event is: ' + event, " filename=",
				filename);
		...
		Async.each(p, function(segment, callback) {
			...
			if (error) {
				console.error("pathRepository: watch error ", error);
			}
			if (node) {
				node.update();
			}
...
PathRepository.prototype.browse = function(list, item, callback) {
	...
	console.log("PathRepository: browseItem=" + itemPath + " path=", path);
